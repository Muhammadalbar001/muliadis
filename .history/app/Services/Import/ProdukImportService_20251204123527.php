...
class ProdukImportService
{
    public function handle(string $filePath)
    {
        ...
        try {
            ...
            foreach ($rows as $rawRow) {
                ...
                // --- HARD FIX: PAKSA JADI INDEX ANGKA [0, 1, 2...] ---
                // Ini mengatasi masalah jika Excel terbaca sebagai Key-Value
                $row = array_values($rawRow);

                // 3. Skip Baris Judul (Cek Kolom 0 = "Cabang")
                if (isset($row[0]) && is_string($row[0]) && strtolower(trim($row[0])) === 'cabang') {
                    continue;
                }

                // 4. Skip Baris Kosong (Jika SKU di Index 2 kosong atau null)
                // Kita trim dulu untuk memastikan spasi kosong dianggap kosong
                if (!isset($row[2]) || trim((string)$row[2]) === '') {
                    continue;
                }

                // 5. MAPPING DATA (Menggunakan Index Pasti)
                Produk::updateOrCreate(
                    ['sku' => trim((string)$row[2])], // Index 2: SKU
                    [
                        // --- IDENTITAS ---
                        'cabang'            => $row[0] ?? null,
                        'ccode'             => $row[1] ?? null,
            ...
            // --- DEBUGGING ERROR ---
            // Jika loop selesai tapi tidak ada data yang masuk, lempar error agar User tahu
            if ($count === 0) {
                if ($totalRowsRead === 0) {
                    throw new \Exception("File Excel terbaca kosong atau format tidak didukung.");
                } else {
                    throw new \Exception("File terbaca ($totalRowsRead baris), tapi tidak ada SKU yang valid ditemukan. Cek Kolom C (Index 2).");
                }
            }
            ...
            throw $e; // Lempar error ke Livewire agar muncul di layar
        }
    }
    ...
    private function cleanNumber($value)
    {
        ...
        // Ubah ke string dulu biar aman
        $value = (string) $value;

        // Hapus karakter selain angka, titik, minus, dan koma
        $clean = preg_replace('/[^0-9.,-]/', '', $value);
        
        // Logika desimal Indonesia (koma) vs Inggris (titik)
    ...
    // Helper: Parse Tanggal (Fix Kutip ' )
    private function parseDate($value)
    {
        if (empty($value) || $value === '-' || $value === 'Blank') return null;

        // Paksa jadi string dan hapus kutip satu di awal
        $value = ltrim((string)$value, "'");

        try {
            // Cek format Excel Numeric (45201)
            if (is_numeric($value)) {
