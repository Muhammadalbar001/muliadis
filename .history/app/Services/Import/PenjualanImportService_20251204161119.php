...
class PenjualanImportService
{
        ...
        try {
            ...
            foreach ($rows as $index => $rawRow) {
                try {
                    ...
                    // --- HELPER LOKAL AMAN ---
                    $val = function ($r, $i) {
                        return $this->val($r, $i); // Panggil method private val()
                    };
                    ...
                    if (is_null($tglPenjualan)) {
                        throw new \Exception("Tanggal Penjualan (Kolom D) tidak valid atau kosong.");
                    }
                    ...
                    Penjualan::updateOrCreate(
                        ...
                        [
                            ...
                            // ANGKA (String murni, minus tetap minus)
                    ...
                    $stats['skipped_error']++;
                    Log::error("Skip Penjualan Baris ke-" . ($index + 1) . " Error: " . $e->getMessage());
                    continue; 
                }
            }
    ...
    // 1. Helper Val: Menangani Object DateTime agar tidak crash (Array to string)
    private function val($row, $index)
    {
        if (!isset($row[$index])) return '';
        $value = $row[$index];

        // FIX: Jika Excel mengembalikan Objek Tanggal, ubah ke string format SQL
    ...
    // 2. Helper Date: Membersihkan tanda kutip (')
    private function date($row, $index)
    {
        $val = $this->val($row, $index);
        
        // FIX: Hapus tanda kutip di awal string (misal: '2025-01-01)
        $val = ltrim($val, "'");
        $val = trim($val);
    ...
    {
        $val = $this->val($row, $index);
        // Hapus kutip jika ada di angka juga
        $val = ltrim($val, "'");
        $val = trim($val);

        if ($val === '-') {
            return '-'; // Return minus murni
        }
        if (empty($val) || $val === '0') {
            return '0';
        }
