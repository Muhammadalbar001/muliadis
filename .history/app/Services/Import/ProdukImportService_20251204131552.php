...
class ProdukImportService
{
    public function handle(string $filePath)
    {
        ...
        try {
            ...
            foreach ($rows as $rawRow) {
                ...
                // --- AMAN DARI ARRAY ---
                // Ambil nilai kolom 0 (Cabang) dan 2 (SKU) dengan aman
                $rawCabang = $row[0] ?? null;
                $rawSku    = $row[2] ?? null;

                // Jika terbaca sebagai array (error source), abaikan atau ambil elemen pertama
                if (is_array($rawCabang)) $rawCabang = json_encode($rawCabang);
                if (is_array($rawSku))    $rawSku    = json_encode($rawSku);
    ...
    // Helper: Ambil String Aman (Cegah Array to String Conversion)
    private function getString($value)
    {
        if (is_array($value)) {
            return ''; // Atau json_encode($value) jika ingin melihat isinya
        }
        return (string) $value;
    }
