...
class ReturImportService
{
        ...
        try {
            ...
            foreach ($rows as $index => $rawRow) {
                try {
                    ...
                    $noReturRaw = $val(1); // Col B
                    $kodeItem   = $val(7); // Col H (Kode Item)
                    ...
                    // 3. Validasi Tanggal Retur (Col D - Index 3)
                    $tglRetur = $this->parseDateLoose($row, 3);

                    // 4. SIMPAN DATA (Update Or Create)
            ...
            DB::rollBack();
            Log::error('Import Retur Fatal: ' . $e->getMessage());
            throw $e;
        }
    }
    
    // --- HELPER SAKTI (Sama seperti Penjualan) ---
    ...
    private function parseDateLoose($row, $index)
    {
        ...
        try {
            ...
            // Terjemahkan Indo ke Inggris jika perlu
