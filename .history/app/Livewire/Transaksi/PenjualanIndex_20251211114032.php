...
class PenjualanIndex extends Component
{
    ...
    public function render()
    {
        ...
        // --- BARU: HITUNG SUMMARY GLOBAL (Sesuai Filter) ---
        // Kita hitung ini SEBELUM melakukan grouping/pagination
        $summary = [
            'total_omzet'  => (clone $query)->sum('total_grand'), // Total Uang
            'total_faktur' => (clone $query)->distinct('trans_no')->count('trans_no'), // Total Faktur Unik
            'total_items'  => (clone $query)->count(), // Total Baris Barang
        ];

        // --- GROUPING LOGIC (Untuk Tabel) ---
        $penjualans = $query
            ->select(
                'trans_no',
                'tgl_penjualan',
                ...
                'cabang',
                DB::raw('MIN(id) as id'),
                DB::raw('SUM(total_grand) as total_invoice'),
        ...
        // Jangan lupa passing variable $summary ke view
