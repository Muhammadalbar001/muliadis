...
class CollectionIndex extends Component
{
    ...
    public function render()
    {
        ...
        // --- SUMMARY STATS ---
        $summary = [
            'total_cair'   => (clone $query)->sum('receive_amount'), // Total Uang Masuk
            'total_bukti'  => (clone $query)->distinct('receive_no')->count('receive_no'),
            'total_faktur' => (clone $query)->count(), // Jumlah Faktur Lunas
        ];

        // --- GROUPING TABLE ---
        // Group by Receive No (No Bukti Pelunasan)
        $collections = $query
            ->select(
                'receive_no', 'tanggal', 'nama_pelanggan', 'sales_name', 'cabang',
                DB::raw('MIN(id) as id'),
                DB::raw('SUM(receive_amount) as total_bayar'),
                DB::raw('COUNT(*) as total_faktur')
            )
            ->groupBy('receive_no', 'tanggal', 'nama_pelanggan', 'sales_name', 'cabang')
