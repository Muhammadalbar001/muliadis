<div class="space-y-6">
    ...
    <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        <div class="flex items-center mb-3 gap-2 text-gray-700">
            ...
            @if($filterCabang || $filterKategori || $filterDivisi || $filterSupplier || $filterStok)
                <button wire:click="$set('filterCabang', '');$set('filterKategori', '');$set('filterDivisi', '');$set('filterSupplier', '');$set('filterStok', '')" 
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div>
                ...
                <select wire:model.live="filterCabang" class="w-full border-gray-200 rounded-lg text-xs focus:ring-indigo-500 focus:border-indigo-500 py-2">
            ...
            <div>
                ...
                <select wire:model.live="filterKategori" class="w-full border-gray-200 rounded-lg text-xs focus:ring-indigo-500 focus:border-indigo-500 py-2">
            ...
            <div>
                ...
                <select wire:model.live="filterDivisi" class="w-full border-gray-200 rounded-lg text-xs focus:ring-indigo-500 focus:border-indigo-500 py-2">
            ...
            <div>
                ...
                <select wire:model.live="filterSupplier" class="w-full border-gray-200 rounded-lg text-xs focus:ring-indigo-500 focus:border-indigo-500 py-2">
            ...
            <div>
                ...
                <select wire:model.live="filterStok" class="w-full border-gray-200 rounded-lg text-xs focus:ring-indigo-500 focus:border-indigo-500 py-2">
        ...
        <div class="flex-1 overflow-auto">
            <table class="w-full text-xs text-left border-collapse whitespace-nowrap">
                <thead class="text-xs text-gray-600 uppercase bg-gray-100 sticky top-0 z-20 shadow-sm">
                    <tr>
                        <th class="px-3 py-3 border-b border-r bg-gray-100 min-w-[50px] text-center sticky left-0 z-30">No</th>
                        <th class="px-3 py-3 border-b border-r bg-gray-100 min-w-[80px] text-center sticky left-[50px] z-30 font-bold">Aksi</th>
                        ...
                        <th class="px-3 py-3 border-b border-r bg-blue-50 text-blue-900 min-w-[80px] text-center font-bold">STOK</th>
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    ...
                    <tr class="transition-colors group {{ $item->is_duplicate ? 'bg-yellow-50 hover:bg-yellow-100' : 'hover:bg-indigo-50' }}">
                        <td class="px-3 py-2 border-r text-center text-gray-500 bg-white sticky left-0 z-10 group-hover:bg-inherit">
                            {{ $produks->firstItem() + $index }}
                        </td>
                        <td class="px-3 py-2 border-r text-center bg-white sticky left-[50px] z-10 shadow-sm group-hover:bg-inherit">
                            <div class="flex justify-center gap-2">
                                <button wire:click="edit({{ $item->id }})" class="text-blue-600 hover:text-blue-800"><i class="fas fa-edit"></i></button>
                            ...
                            @if($item->is_duplicate) <div class="text-[9px] text-red-600 font-bold mt-0.5">DUPLIKAT</div> @endif
                        </td>
                        ...
                        <td class="px-3 py-2 border-r font-medium text-gray-800 truncate max-w-[250px]" title="{{ $item->name_item }}">{{ $item->name_item }}</td>
                        <td class="px-3 py-2 border-r text-red-500">{{ $item->expired_date ? \Carbon\Carbon::parse($item->expired_date)->format('d-m-Y') : '-' }}</td>

                        <td class="px-3 py-2 border-r text-center font-bold {{ (float)str_replace(',', '', $item->stok) > 0 ? 'text-green-600' : 'text-red-600' }}">
                            {{ $item->stok }}
                        </td>
                        ...
                        <td class="px-3 py-2 border-r text-right font-bold {{ str_contains($item->margin, '-') ? 'text-red-600' : 'text-green-600' }}">{{ $item->margin }}</td>

                        <td class="px-3 py-2 border-r truncate max-w-[150px]" title="{{ $item->supplier }}">{{ $item->supplier }}</td>
    ...
    @if($isImportOpen)
        <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 bg-gray-900 bg-opacity-75 transition-opacity backdrop-blur-sm" wire:click="closeImportModal"></div>
                <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>
                <div class="relative inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md w-full">
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6">
                        <h3 class="text-lg leading-6 font-bold text-gray-900 mb-4">Import Master Produk</h3>
                        <div class="mt-2 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:bg-gray-50 relative cursor-pointer">
                             <label for="file-upload-prod-{{ $iteration }}" class="relative cursor-pointer font-medium text-emerald-600 hover:text-emerald-500">
                                <span>Pilih File</span>
                                <input id="file-upload-prod-{{ $iteration }}" wire:model="file" type="file" class="sr-only">
                            </label>
                        </div>
                        <div wire:loading wire:target="file" class="w-full mt-2 text-center text-sm text-emerald-600">Mengupload...</div>
                        @error('file') <div class="mt-2 text-sm text-red-600">{{ $message }}</div> @enderror
                    </div>
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
                        <button wire:click="import" wire:loading.attr="disabled" class="w-full inline-flex justify-center rounded-lg border border-transparent shadow-sm px-4 py-2 bg-emerald-600 text-base font-medium text-white hover:bg-emerald-700 sm:ml-3 sm:w-auto sm:text-sm disabled:opacity-50">
                            <span wire:loading.remove wire:target="import">Import</span>
                            <span wire:loading wire:target="import">Memproses...</span>
                        </button>
                        <button wire:click="closeImportModal" class="mt-3 w-full inline-flex justify-center rounded-lg border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 sm:mt-0 sm:w-auto sm:text-sm">Batal</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    @if($isInputOpen)
        <div class="fixed inset-0 z-50 overflow-y-auto">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 bg-gray-900 bg-opacity-50 transition-opacity backdrop-blur-sm" wire:click="closeInputModal"></div>
                <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>
                <div class="relative inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
                    ...
                    <div class="px-6 py-4 space-y-4">
                        <div><label class="block text-sm text-gray-700">SKU</label><input type="text" wire:model="kode_item" class="w-full rounded border-gray-300"></div>
                        <div><label class="block text-sm text-gray-700">Nama</label><input type="text" wire:model="nama_item" class="w-full rounded border-gray-300"></div>
                    </div>
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <button wire:click="store" class="w-full inline-flex justify-center rounded bg-indigo-600 text-white px-4 py-2 sm:ml-3 sm:w-auto">Simpan</button>
                        <button wire:click="closeInputModal" class="mt-3 w-full inline-flex justify-center rounded border border-gray-300 bg-white px-4 py-2 sm:mt-0 sm:w-auto">Batal</button>
                    </div>
                </div>
            </div>
        </div>
