...
class ProdukIndex extends Component
{
    ...
    public function import()
    {
        ...
        try {
            ...
            // Clear cache filter
            Cache::forget('opt_cabang'); Cache::forget('opt_kategori'); Cache::forget('opt_divisi'); Cache::forget('opt_supplier');

            $uniqueCount = $stats['processed'] - $stats['duplicates_marked'];
            $message = "Selesai! Total: " . number_format($stats['total_rows']) . "\nUnik: " . number_format($uniqueCount) . "\nDuplikat: " . number_format($stats['duplicates_marked']);
            
            $this->dispatch('show-toast', ['type' => 'success', 'title' => 'Sukses', 'message' => $message]);
        } catch (\Exception $e) {
