...
class ProdukIndex extends Component
{
    ...
    // --- Properties Tampilan ---
    public $search = '';
    public $isInputOpen = false;  // Modal CRUD Manual
    public $isImportOpen = false; // Modal Import Excel
    ...
    public function import()
    {
        ...
        try {
            ...
            // Data bersih = Total yang dikirim ke DB - Duplikat
            $uniqueCount = $stats['processed'] - $stats['duplicates'];

            $message = "📂 Total Baris: " . number_format($stats['total_rows']) . "\n" .
                       "✅ Masuk (Unik): " . number_format($uniqueCount) . "\n" .
                       "♻️ Duplikat (Update): " . number_format($stats['duplicates']);

            if ($stats['skipped_empty'] > 0) {
                $message .= "\n⚠️ Skipped: " . number_format($stats['skipped_empty']);
            }

            // Dispatch Event Browser untuk Toast
