...
class RekapCollectionIndex extends Component
{
    ...
    public function render()
    {
        ...
        if ($this->search) {
            $query->where(function($q) {
                $q->where('receive_no', 'like', '%'.$this->search.'%')
                  ->orWhere('outlet_name', 'like', '%'.$this->search.'%') // Pelanggan
        ...
        $optCabang  = Cache::remember('coll_cabang', 3600, fn() => Collection::select('cabang')->distinct()->pluck('cabang'));
        $optPenagih = Cache::remember('coll_penagih', 3600, fn() => Collection::select('penagih')->distinct()->pluck('penagih'));
