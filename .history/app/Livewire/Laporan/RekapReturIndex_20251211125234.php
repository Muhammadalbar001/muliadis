...
class RekapReturIndex extends Component
{
    ...
    private function applyFilters($query)
    {
        if ($this->search) {
            $query->where(function($q) {
                $q->where('no_retur', 'like', '%'.$this->search.'%')->orWhere('nama_pelanggan', 'like', '%'.$this->search.'%');
            });
        }
    ...
    public function render()
    {
        $query = Retur::query();
        $this->applyFilters($query);
        $returs = $query->orderBy('tgl_retur', 'desc')->paginate(20); // Rename variable
