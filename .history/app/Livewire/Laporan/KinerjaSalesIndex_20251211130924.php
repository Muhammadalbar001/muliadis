...
class KinerjaSalesIndex extends Component
{
    ...
    public function export()
    {
        ...
        $suppliers = $data['topSuppliers']; // Ambil daftar supplier
        $matrix = $data['matrixSupplier'];  // Ambil data angka

        $writer = SimpleExcelWriter::streamDownload('Rapor_Kinerja_Sales_' . $this->bulan . '.xlsx');

        // Header Custom untuk Excel (Karena kolom supplier dinamis)
        $header = [
            'Nama Sales', 'Cabang', 
            'Target (Rp)', 'Realisasi (Rp)', 'Ach (%)', 'Kurang (Rp)',
            'Total AR', 'AR Lancar', 'AR Macet', 'Rasio Macet %',
            'Target OA', 'Real OA', 'Ach OA %', 'EC'
        ];

        // Tambahkan Header Supplier Dinamis ke Excel
        ...
        foreach ($laporan as $row) {
            $rowData = [
                $row['nama'], $row['cabang'],
                $row['target_ims'], $row['real_ims'], $row['persen_ims'], $row['defisit'],
                $row['ar_total'], $row['ar_lancar'], $row['ar_macet'], $row['persen_macet'],
                $row['target_oa'], $row['real_oa'], $row['persen_oa'], $row['ec'],
            ];

            // Tambahkan Data Supplier Dinamis per Baris
            foreach($suppliers as $supp) {
    ...
    private function getDataLaporan()
    {
        ...
        // --- UPDATE: PIVOT SUPPLIER (TAMPIL SEMUA) ---
        // Hapus 'limit(10)' agar semua supplier muncul
