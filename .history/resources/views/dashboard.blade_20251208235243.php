<div class="min-h-screen space-y-6 font-jakarta pb-10" 
    ...
    <div class="sticky top-0 z-40 backdrop-blur-md bg-white/90 p-4 rounded-b-2xl shadow-sm border-b border-slate-200 transition-all duration-300">
        <div class="flex flex-col lg:flex-row gap-4 items-center justify-between">
            ...
            <div class="flex gap-2 items-center">
                ...
                <select wire:model.live="filterCabang" class="border-slate-200 rounded-lg text-xs font-bold text-slate-700 focus:ring-indigo-500 py-2 w-32">
    ...
    <div x-show="activeTab === 'ranking'" x-transition.opacity.duration.300ms class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div x-show="activeTab === 'salesman'" x-transition.opacity.duration.300ms class="grid grid-cols-1 gap-6">
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            <h4 class="font-bold text-lg text-indigo-900 mb-4 pb-2 border-b border-slate-50">🎯 Top 10 Sales Performance (Omzet)</h4>
    ...
    const renderCharts = (data) => {
        ...
        charts.arColl = new Chart(ctxAC, {
            ...
            data: {
                labels: data.dates,
                datasets: [
                    { label: 'Piutang', data: data.ar, backgroundColor: '#f97316', borderRadius: 3 },
                    { label: 'Collection', data: data.coll, backgroundColor: '#10b981', borderRadius: 3 }
                ]
            },
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'top' } }, scales: { y: { beginAtZero: true } } }
        });
        ...
        charts.ims = new Chart(document.getElementById('imsChart'), {
            ...
            data: {
                ...
                datasets: [
                    { label: 'Target', data: data.salesTargetIMS, backgroundColor: '#e2e8f0', borderRadius: 4 },
                    { label: 'Realisasi', data: data.salesRealIMS, backgroundColor: '#4f46e5', borderRadius: 4 }
                ]
            },
            options: { 
                responsive: true, 
