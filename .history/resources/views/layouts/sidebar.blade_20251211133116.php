<aside
    class="fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 border-r border-slate-800 text-white transition-transform duration-300 flex flex-col lg:translate-x-0"
    ...
    <div
        ...
        <button @click="sidebarOpen = false"
            class="lg:hidden text-slate-400 hover:text-white transition-colors p-1 focus:outline-none">
        ...
        <div x-data="{ open: {{ request()->routeIs('laporan.rekap*') ? 'true' : 'false' }} }">
            ...
            <div x-show="open" x-transition x-cloak class="space-y-1 mt-1 px-2 border-l border-slate-700 ml-6">
                ...
                <a href="{{ route('laporan.rekap-penjualan') }}"
                    ...
                    <div
                        class="w-1.5 h-1.5 rounded-full mr-3 {{ request()->routeIs('laporan.rekap-penjualan') ? 'bg-emerald-400' : 'bg-slate-600' }}">
                    </div>
                    Penjualan
                </a>
                ...
                <a href="{{ route('laporan.rekap-retur') }}"
                    ...
                    <div
                        class="w-1.5 h-1.5 rounded-full mr-3 {{ request()->routeIs('laporan.rekap-retur') ? 'bg-rose-400' : 'bg-slate-600' }}">
                    </div>
                    Retur
                </a>
                ...
                <a href="{{ route('laporan.rekap-ar') }}"
                    ...
                    <div
                        class="w-1.5 h-1.5 rounded-full mr-3 {{ request()->routeIs('laporan.rekap-ar') ? 'bg-orange-400' : 'bg-slate-600' }}">
                    </div>
                    Piutang
                </a>
                ...
                <a href="{{ route('laporan.rekap-collection') }}"
                    ...
                    <div
                        class="w-1.5 h-1.5 rounded-full mr-3 {{ request()->routeIs('laporan.rekap-collection') ? 'bg-cyan-400' : 'bg-slate-600' }}">
                    </div>
                    Collection
                </a>
            </div>
    ...
    <div class="p-4 border-t border-slate-800 bg-slate-950 flex-none">
        <div class="flex items-center justify-between gap-3">
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit"
                    class="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-rose-400 hover:bg-rose-900/20 border border-transparent rounded-xl transition-all"
                    title="Logout">
