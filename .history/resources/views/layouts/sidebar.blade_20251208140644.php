<aside class="fixed inset-y-0 left-0 z-50 bg-slate-900 text-white transition-all duration-300 flex flex-col shadow-2xl"
    ...
    <div class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
        ...
        <div class="mt-6 mb-2 px-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center justify-between"
            ...
            <i class="fas fa-keyboard text-xs opacity-50"></i>
        </div>
        ...
        <a href="{{ route('transaksi.penjualan') }}"
           ...
           {{ request()->routeIs('transaksi.penjualan') ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
            <i
                class="fas fa-cash-register w-6 text-center {{ request()->routeIs('transaksi.penjualan') ? 'text-white' : 'text-slate-500 group-hover:text-white' }}"></i>
            <span class="ml-3 font-medium text-sm" :class="!sidebarOpen && 'hidden'">Input Penjualan</span>
        </a>
        ...
        <a href="{{ route('transaksi.retur') }}"
           ...
           {{ request()->routeIs('transaksi.retur') ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
            <i
                class="fas fa-rotate-left w-6 text-center {{ request()->routeIs('transaksi.retur') ? 'text-white' : 'text-slate-500 group-hover:text-white' }}"></i>
            <span class="ml-3 font-medium text-sm" :class="!sidebarOpen && 'hidden'">Input Retur</span>
        </a>
        ...
        <a href="{{ route('transaksi.ar') }}"
           ...
           {{ request()->routeIs('transaksi.ar') ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
            <i
                class="fas fa-file-invoice w-6 text-center {{ request()->routeIs('transaksi.ar') ? 'text-white' : 'text-slate-500 group-hover:text-white' }}"></i>
            <span class="ml-3 font-medium text-sm" :class="!sidebarOpen && 'hidden'">Input Data AR</span>
        </a>
        ...
        <a href="{{ route('transaksi.collection') }}"
           ...
           {{ request()->routeIs('transaksi.collection') ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
            <i
                class="fas fa-wallet w-6 text-center {{ request()->routeIs('transaksi.collection') ? 'text-white' : 'text-slate-500 group-hover:text-white' }}"></i>
            <span class="ml-3 font-medium text-sm" :class="!sidebarOpen && 'hidden'">Input Collection</span>
        </a>

        <div class="mt-6 mb-2 px-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center justify-between"
            :class="!sidebarOpen && 'hidden'">
            <span>Laporan (Rekap)</span>
            <i class="fas fa-table text-xs opacity-50"></i>
        </div>

        <a href="{{ route('laporan.rekap-penjualan') }}"
            class="flex items-center px-3 py-2.5 rounded-lg transition-colors group relative
           {{ request()->routeIs('laporan.rekap-penjualan') ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
            <i
                class="fas fa-list-alt w-6 text-center {{ request()->routeIs('laporan.rekap-penjualan') ? 'text-white' : 'text-slate-500 group-hover:text-white' }}"></i>
            <span class="ml-3 font-medium text-sm" :class="!sidebarOpen && 'hidden'">Rekap Penjualan</span>
        </a>

        <a href="{{ route('laporan.rekap-retur') }}"
            class="flex items-center px-3 py-2.5 rounded-lg transition-colors group relative
           {{ request()->routeIs('laporan.rekap-retur') ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
            <i
                class="fas fa-list-alt w-6 text-center {{ request()->routeIs('laporan.rekap-retur') ? 'text-white' : 'text-slate-500 group-hover:text-white' }}"></i>
            <span class="ml-3 font-medium text-sm" :class="!sidebarOpen && 'hidden'">Rekap Retur Jual</span>
        </a>

        <a href="{{ route('laporan.rekap-ar') }}"
            class="flex items-center px-3 py-2.5 rounded-lg transition-colors group relative
           {{ request()->routeIs('laporan.rekap-ar') ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
            <i
                class="fas fa-list-alt w-6 text-center {{ request()->routeIs('laporan.rekap-ar') ? 'text-white' : 'text-slate-500 group-hover:text-white' }}"></i>
            <span class="ml-3 font-medium text-sm" :class="!sidebarOpen && 'hidden'">Rekap Piutang (AR)</span>
        </a>

        <a href="{{ route('laporan.rekap-collection') }}"
            class="flex items-center px-3 py-2.5 rounded-lg transition-colors group relative
           {{ request()->routeIs('laporan.rekap-collection') ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
            <i
                class="fas fa-list-alt w-6 text-center {{ request()->routeIs('laporan.rekap-collection') ? 'text-white' : 'text-slate-500 group-hover:text-white' }}"></i>
