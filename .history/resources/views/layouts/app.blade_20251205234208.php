                ...
                <div x-data="{ open: {{ request()->routeIs('master.*') ? 'true' : 'false' }} }" class="space-y-1">
                    ...
                    <div x-show="open && desktopSidebarOpen" x-collapse x-cloak class="space-y-1 pl-10 pr-2">
                        ...
                        <a href="{{ route('master.sales') }}"
                            class="block px-3 py-2 text-sm rounded-md transition-colors {{ request()->routeIs('master.sales') ? 'text-indigo-400 bg-slate-800' : 'hover:text-white hover:bg-slate-800' }}">Sales
                            Team</a>
                        <a href="{{ route('master.users') }}"
                            class="block px-3 py-2 text-sm rounded-md transition-colors {{ request()->routeIs('master.users') ? 'text-indigo-400 bg-slate-800' : 'hover:text-white hover:bg-slate-800' }}">Users</a>
                    </div>
                </div>
                ...
                <div x-data="{ open: {{ request()->routeIs('transaksi.*') ? 'true' : 'false' }} }" class="space-y-1">
                    ...
                    <div x-show="open && desktopSidebarOpen" x-collapse x-cloak class="space-y-1 pl-10 pr-2">
                        <a href="{{ route('transaksi.penjualan') }}"
                            class="block px-3 py-2 text-sm rounded-md transition-colors {{ request()->routeIs('transaksi.penjualan') ? 'text-indigo-400 bg-slate-800' : 'hover:text-white hover:bg-slate-800' }}">Order
                            Penjualan</a>
                        <a href="{{ route('transaksi.retur') }}"
                            class="block px-3 py-2 text-sm rounded-md transition-colors {{ request()->routeIs('transaksi.retur') ? 'text-indigo-400 bg-slate-800' : 'hover:text-white hover:bg-slate-800' }}">Retur
                            Barang</a>
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-hidden bg-gray-50">
            ...
            <header
                ...
                <div class="flex items-center gap-4">
                    ...
                    <button
                        class="relative p-2 text-gray-400 hover:text-indigo-600 transition-colors rounded-full hover:bg-indigo-50">
                        <i class="fas fa-bell fa-lg"></i>
                        <span
                            class="absolute top-1.5 right-2 w-2 h-2 bg-red-500 rounded-full border border-white"></span>
                    </button>
                    ...
                    <div x-data="{ dropdownOpen: false }" class="relative">
                        <button @click="dropdownOpen = !dropdownOpen"
                            ...
                            <div
                                class="w-10 h-10 rounded-full bg-gradient-to-tr from-indigo-500 to-blue-500 p-0.5 shadow-md hover:shadow-lg transition-all cursor-pointer">
                                <div
                                    class="w-full h-full rounded-full bg-white flex items-center justify-center text-indigo-600 font-bold text-lg">
            ...
            <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-50 p-6 md:p-8">
                ...
                <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000)"
                    ...
                    <button @click="show = false" class="text-green-400 hover:text-green-600"><i
