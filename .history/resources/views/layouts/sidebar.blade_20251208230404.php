<aside
    ...
    <div class="flex-1 overflow-y-auto py-4 px-3 space-y-1 custom-scrollbar">
        ...
        <a href="{{ route('laporan.rekap-penjualan') }}"
            class="flex items-center px-4 py-2.5 rounded-xl transition-all duration-200 group text-sm font-medium {{ request()->routeIs('laporan.rekap-penjualan') ? 'bg-gradient-to-r from-emerald-600 to-teal-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
            <i class="fas fa-list-alt w-5 text-center"></i> <span class="ml-3">Rekap Penjualan</span>
        </a>
        <a href="{{ route('laporan.rekap-retur') }}"
            class="flex items-center px-4 py-2.5 rounded-xl transition-all duration-200 group text-sm font-medium {{ request()->routeIs('laporan.rekap-retur') ? 'bg-gradient-to-r from-rose-600 to-pink-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
            <i class="fas fa-rotate-left w-5 text-center"></i> <span class="ml-3">Rekap Retur Jual</span>
        </a>
        <a href="{{ route('laporan.rekap-ar') }}"
            class="flex items-center px-4 py-2.5 rounded-xl transition-all duration-200 group text-sm font-medium {{ request()->routeIs('laporan.rekap-ar') ? 'bg-gradient-to-r from-orange-500 to-amber-500 text-white shadow-md' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
            <i class="fas fa-file-invoice w-5 text-center"></i> <span class="ml-3">Rekap Piutang (AR)</span>
        </a>
        <a href="{{ route('laporan.rekap-collection') }}"
            class="flex items-center px-4 py-2.5 rounded-xl transition-all duration-200 group text-sm font-medium {{ request()->routeIs('laporan.rekap-collection') ? 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
            <i class="fas fa-money-check-dollar w-5 text-center"></i> <span class="ml-3">Rekap Collection</span>
        </a>
        ...
        <a href="{{ route('laporan.kinerja-sales') }}"
            class="flex items-center px-4 py-2.5 rounded-xl transition-all duration-200 group text-sm font-medium {{ request()->routeIs('laporan.kinerja-sales') ? 'bg-gradient-to-r from-violet-600 to-purple-600 text-white shadow-lg shadow-purple-500/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
            <i class="fas fa-chart-line w-5 text-center"></i> <span class="ml-3">Rapor Kinerja Sales</span>
        </a>
    ...
    <div class="p-4 border-t border-slate-800 bg-slate-950 flex-none">
        <div class="flex items-center justify-between gap-2">
            ...
            <a href="{{ route('profile.edit') }}"
                class="flex items-center gap-3 flex-1 min-w-0 group hover:bg-slate-900 rounded-lg p-1 transition-colors"
                title="Lihat Profil">
                <div
                    class="w-9 h-9 rounded-full bg-slate-700 flex items-center justify-center text-sm font-bold text-white uppercase border border-slate-600 group-hover:border-indigo-500 transition-colors">
                    {{ substr(Auth::user()->name ?? 'A', 0, 1) }}
                </div>
                <div class="flex-1 min-w-0 text-left">
                    <p class="text-sm font-medium text-white truncate group-hover:text-indigo-400 transition-colors">
                        {{ Auth::user()->name ?? 'User' }}</p>
                    <p class="text-[10px] text-slate-400 truncate">Edit Profil</p>
                </div>
            </a>
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit"
                    class="text-slate-400 hover:text-red-400 hover:bg-red-500/10 p-2 rounded-lg transition-all"
                    title="Logout">
