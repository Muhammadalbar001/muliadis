<div class="space-y-6 font-jakarta">
    ...
    <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
        <div class="flex flex-col md:flex-row gap-4 items-end">
            ...
            <button wire:click="resetFilter"
                class="w-full md:w-auto px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-500 rounded-xl transition-colors h-[42px]">
                    ...
                    <tr class="hover:bg-emerald-50/20 transition-colors group">
                        ...
                        <td class="px-6 py-3 border-r border-slate-100">
                            <button wire:click="openDetail('{{ $item->trans_no }}')"
                                ...
                                <i
                                    class="fas fa-external-link-alt text-[10px] opacity-0 group-hover/link:opacity-100 transition-opacity"></i>
                            </button>
                        </td>
                        ...
                        <td
                            class="px-6 py-3 text-center sticky right-0 bg-white group-hover:bg-emerald-50/20 transition-colors">
    ...
    <div class="fixed inset-0 z-[70] overflow-y-auto" role="dialog" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div
                class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl w-full border border-white/20">
                    ...
                    <table class="w-full text-xs text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-slate-50">
                                <td class="px-4 py-2 font-mono text-slate-500">{{ $d->sku ?? $d->kode_item ?? '-' }}
                                </td>
                                <td class="px-4 py-2 font-bold text-slate-700">{{ $d->nama_item ?? 'Item Tanpa Nama' }}
                                </td>
                                <td class="px-4 py-2 text-right">{{ number_format($d->qty, 0, ',', '.') }}</td>
                                <td class="px-4 py-2 text-right">{{ number_format($d->nilai_jual_net, 0, ',', '.') }}
                                </td>
                ...
                <div class="bg-slate-50 px-6 py-4 flex justify-end">
                    <button wire:click="closeDetail"
                        class="px-5 py-2 bg-white border border-slate-300 text-slate-700 font-bold rounded-xl text-xs hover:bg-slate-100 transition">
                        Tutup
                    </button>
                </div>
            </div>
