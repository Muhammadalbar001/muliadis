    ...
    <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div class="w-full md:w-auto bg-blue-50 p-3 rounded-lg border border-blue-200"
                ...
                x-on:livewire-upload-start="isUploading = true; progress = 0; hasFile = true"
                x-on:livewire-upload-finish="isUploading = false"
                x-on:livewire-upload-error="isUploading = false; hasFile = false"
                x-on:livewire-upload-progress="progress = $event.detail.progress">

                <!-- Gunakan DIV biasa agar tidak ada form submit otomatis -->
                <div class="flex flex-col gap-2">
                    <div class="flex flex-col md:flex-row gap-2 items-center">
                        <div class="relative">
                            <!-- Input File dengan ID Iteration untuk Reset -->
                        ...
                        <!-- Tombol Import -->
                        <button type="button" wire:click="import" :disabled="!hasFile || isUploading"
                            ...
                            <!-- 1. Teks Default -->
                            <span x-show="!isUploading && !$wire.get('file')" wire:loading.remove>
                                <i class="fas fa-upload"></i> Import Penjualan
                            </span>

                            <!-- 2. Teks Saat Uploading (Browser -> Server) -->
                            <span x-show="isUploading" x-cloak>
                                <i class="fas fa-spinner fa-spin"></i> Uploading... <span
                                    x-text="progress + '%'"></span>
                            </span>

                            <!-- 3. Teks Saat Memproses (Server -> DB) -->
                            <!-- Muncul hanya jika upload selesai tapi wire:loading aktif -->
                            <span wire:loading wire:target="import">
                                <i class="fas fa-cog fa-spin"></i> Memproses Data...
                            </span>
                        </button>
                    </div>
                    ...
                    <div x-show="isUploading"
                        class="w-full bg-gray-200 rounded-full h-3 mt-1 border border-gray-300 overflow-hidden">
                        <div class="bg-blue-600 h-3 rounded-full transition-all duration-300 text-[9px] text-white text-center leading-3"
                            :style="`width: ${progress}%`">
                            <span x-text="progress + '%'"></span>
                        </div>
                    </div>

                    <div x-show="isUploading" class="text-xs text-blue-600 text-center animate-pulse">
