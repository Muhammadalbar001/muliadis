<div class="space-y-4">
    ...
    <div>
        ...
        <div class="bg-green-100 text-green-700 p-4 rounded mb-4 flex justify-between items-center shadow">
        ...
        <div class="bg-red-100 text-red-700 p-4 rounded mb-4 flex justify-between items-center shadow">
            <div><i class="fas fa-exclamation-triangle"></i> {{ session('error') }}</div>
            <button type="button" onclick="this.parentElement.remove()">&times;</button>
        </div>
        @endif
    ...
    <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <!-- UPLOAD AREA (AlpineJS + Livewire Integration) -->
            @if (!$isLaporanMode)
                    ...
                    <div class="flex flex-col md:flex-row gap-2 items-center">
                        ...
                        <button type="button" wire:click="import" wire:loading.attr="disabled" :disabled="uploading"
                            ...
                            <span wire:loading wire:target="import">
                                <i class="fas fa-cog fa-spin"></i> Memproses DB...
                            </span>
                        </button>
                    </div>

                    <!-- Progress Bar (Muncul saat file dikirim ke server) -->
                    <div x-show="uploading" class="w-full bg-gray-200 rounded-full h-2.5 mt-1">
                        <div class="bg-blue-600 h-2.5 rounded-full transition-all duration-300"
                            :style="`width: ${progress}%`"></div>
                        <div class="text-xs text-center text-blue-700 mt-1" x-text="`${progress}% Uploaded`"></div>
                    </div>

                    <!-- Loading Indicator (Muncul saat PHP sedang import database) -->
                    <div wire:loading wire:target="import"
                        class="text-xs text-green-700 text-center mt-1 font-bold animate-pulse">
                        Sedang menyimpan data ke database, mohon tunggu...
                    </div>
                </div>
            </div>
            @endif
