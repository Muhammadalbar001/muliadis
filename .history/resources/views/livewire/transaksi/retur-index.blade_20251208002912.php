<div class="space-y-6">
    ...
    <div
        ...
        <div class="w-full md:w-3/4 grid grid-cols-1 md:grid-cols-4 gap-3">
            <div class="relative md:col-span-1">
                <input wire:model.live.debounce.300ms="search" type="text"
                    class="pl-9 pr-4 py-2 w-full border-gray-200 rounded-lg text-sm focus:ring-red-500 focus:border-red-500 placeholder-gray-400"
            ...
            <div x-data="{ open: false }" class="relative">
                ...
                <div x-show="open"
                    ...
                    <label class="flex items-center px-2 py-1 hover:bg-red-50 rounded cursor-pointer">
            ...
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open" @click.outside="open = false"
                    class="w-full bg-white border border-gray-200 text-gray-700 px-3 py-2 rounded-lg text-sm flex items-center justify-between hover:bg-gray-50">
                    <span
                        class="truncate">{{ empty($filterStatus) ? 'Semua Status' : count($filterStatus) . ' Terpilih' }}</span>
                    <i class="fas fa-chevron-down text-xs text-gray-400"></i>
                </button>
                <div x-show="open"
                    class="absolute z-10 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg p-2 max-h-48 overflow-y-auto"
        ...
        <button wire:click="openImportModal"
            class="px-5 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-bold rounded-lg shadow-sm transition-all flex items-center">
        ...
        <div class="flex-1 overflow-auto">
            <table class="w-full text-xs text-left border-collapse whitespace-nowrap">
                <thead class="bg-gray-50 text-gray-600 font-bold border-b border-gray-200 uppercase">
                    <tr>
                        <th class="px-4 py-3 border-r min-w-[100px]">Cabang</th>
                        <th class="px-4 py-3 border-r min-w-[120px]">No Retur</th>
                        <th class="px-4 py-3 border-r">Tgl Retur</th>
                        <th class="px-4 py-3 border-r min-w-[150px]">Pelanggan</th>
                        <th class="px-4 py-3 border-r min-w-[200px]">Item Barang</th>
                        <th class="px-4 py-3 border-r text-center">Qty</th>
                        <th class="px-4 py-3 border-r text-right bg-red-50 text-red-900">Total Nilai</th>
                        <th class="px-4 py-3 border-r">Status</th>
                        <th class="px-4 py-3 border-r">No Invoice Asal</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-red-50 transition-colors">
                        <td class="px-4 py-2 border-r text-red-600 font-medium">{{ $item->cabang }}</td>
                        <td class="px-4 py-2 border-r font-mono font-bold text-gray-800">{{ $item->no_retur }}</td>
                        ...
                        <td class="px-4 py-2 border-r truncate max-w-xs" title="{{ $item->nama_item }}">
                            {{ $item->nama_item }}</td>
                        <td class="px-4 py-2 border-r text-center font-bold">{{ $item->qty }}</td>
                        <td class="px-4 py-2 border-r text-right font-bold text-red-700">
                        ...
                        <td class="px-4 py-2 border-r text-gray-500">{{ $item->no_inv }}</td>
                    ...
                    @empty
                    <tr>
                        <td colspan="10" class="px-6 py-12 text-center text-gray-400">Belum ada data retur.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        <div class="px-4 py-3 border-t bg-gray-50">{{ $returs->links() }}</div>
    </div>
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6">
                    <h3 class="text-lg font-bold text-gray-900 mb-4">Import Retur</h3>
                    <div
                        class="mt-2 border-2 border-dashed rounded-lg p-6 text-center cursor-pointer hover:bg-gray-50 relative">
                        <input type="file" wire:model="file"
                            class="absolute inset-0 w-full h-full opacity-0 cursor-pointer">
                        <i class="fas fa-cloud-upload-alt text-3xl text-red-500 mb-2"></i>
                        <span class="block text-sm font-medium text-gray-700">Klik Upload Excel</span>
                    </div>
                    <div wire:loading wire:target="file" class="w-full mt-2 text-center text-xs text-red-600">
                        Uploading...</div>
                </div>
                <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse gap-2">
                    <button wire:click="import" wire:loading.attr="disabled"
                        class="w-full sm:w-auto bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 disabled:opacity-50">Import</button>
