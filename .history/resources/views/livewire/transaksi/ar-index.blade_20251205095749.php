    ...
    <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div class="w-full md:w-1/3 relative">
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari No Faktur / Pelanggan..."
                    ...
                    <div class="flex flex-col md:flex-row gap-2 items-center">
                        ...
                        <button type="button" wire:click="import" wire:loading.attr="disabled"
                            ...
                            <span wire:loading.remove wire:target="import" x-show="!isUploading"><i
                                    class="fas fa-upload"></i> Import AR</span>
        ...
        <div class="flex-1 overflow-auto min-w-full">
            <table class="w-full text-sm text-left border-collapse">
                <thead class="text-xs text-gray-700 uppercase bg-gray-100 sticky top-0 z-10 shadow-sm">
                    <tr>
                        <th class="px-4 py-3 border-b">Tgl Faktur</th>
                        <th class="px-4 py-3 border-b">No. Faktur</th>
                        <th class="px-4 py-3 border-b">Jatuh Tempo</th>
                        <th class="px-4 py-3 border-b">Pelanggan</th>
                        <th class="px-4 py-3 border-b">Sales</th>
                        <th class="px-4 py-3 border-b text-center">Umur (Hari)</th>
                        <th class="px-4 py-3 border-b text-right">Total Nilai</th>
                        <th class="px-4 py-3 border-b text-right">Sisa Piutang</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-blue-50">
                        ...
                        <td class="px-4 py-3 font-bold text-blue-700">{{ $item->no_penjualan }}</td>
                        <td class="px-4 py-3 text-red-600 font-medium">
                            {{ $item->jatuh_tempo ? \Carbon\Carbon::parse($item->jatuh_tempo)->format('d/m/Y') : '-' }}
                        </td>
                        <td class="px-4 py-3 font-medium text-gray-900">{{ $item->pelanggan_name }}</td>
                        <td class="px-4 py-3 text-gray-600">{{ $item->sales_name }}</td>
                        <td class="px-4 py-3 text-center"><span
                                class="bg-gray-200 text-gray-800 text-xs font-bold px-2 py-0.5 rounded">{{ $item->umur_piutang }}</span>
                        </td>
                        <td class="px-4 py-3 text-right text-gray-500">{{ $item->total_nilai }}</td>
                        <td class="px-4 py-3 text-right font-bold text-red-700">{{ $item->nilai }}</td>
                        <td class="px-4 py-3 text-center"><span
                                class="bg-yellow-100 text-yellow-800 text-xs font-medium px-2.5 py-0.5 rounded">{{ $item->status }}</span>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="9" class="px-6 py-12 text-center text-gray-400">Belum ada data Piutang.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        <div class="p-3 border-t bg-gray-50">{{ $ar->links() }}</div>
    </div>
</div>