            ...
            <div class="flex flex-col md:flex-row items-center gap-6 w-full lg:w-auto">
                ...
                <div class="flex p-1 bg-slate-100 rounded-xl overflow-x-auto max-w-full">
                    ...
                    <button @click="activeTab = 'salesman'"
                        ...
                        class="px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 whitespace-nowrap"><i
                            class="fas fa-user-tie"></i> Kinerja Sales</button>
                </div>
            </div>
            <div class="flex flex-col sm:flex-row gap-2 w-full lg:w-auto items-center">
                <div class="flex items-center bg-white border border-slate-200 rounded-lg px-2 py-1 shadow-sm">
                    <input type="date" wire:model.live="startDate"
                        class="border-none text-xs font-bold text-slate-700 focus:ring-0 p-1 bg-transparent"><span
                        class="text-slate-300">-</span><input type="date" wire:model.live="endDate"
                ...
                <div class="relative w-full sm:w-40" x-data="{ open: false }">
                    <button @click="open = !open" @click.outside="open = false"
                        class="w-full bg-white border border-slate-200 text-slate-700 px-3 py-2 rounded-lg text-xs font-bold flex items-center justify-between shadow-sm hover:bg-slate-50"><span
                            class="truncate">{{ empty($filterCabang) ? 'Semua Cabang' : count($filterCabang).' Dipilih' }}</span><i
                            class="fas fa-chevron-down text-[10px] text-slate-400"></i></button>
                    <div x-show="open"
                        class="absolute z-50 mt-1 w-full bg-white border border-slate-200 rounded-lg shadow-xl p-2 max-h-48 overflow-y-auto"
                        style="display: none;">
                        @foreach($optCabang as $cab) <label
                            class="flex items-center px-2 py-1.5 hover:bg-slate-50 rounded cursor-pointer"><input
                                type="checkbox" value="{{ $cab }}" wire:model.live="filterCabang"
                                class="rounded border-slate-300 text-indigo-600 mr-2 h-3 w-3"><span
                                class="text-xs text-slate-600">{{ $cab }}</span></label> @endforeach
                    </div>
                </div>
                <div class="relative w-full sm:w-40" x-data="{ open: false }">
                    <button @click="open = !open" @click.outside="open = false"
                        class="w-full bg-white border border-slate-200 text-slate-700 px-3 py-2 rounded-lg text-xs font-bold flex items-center justify-between shadow-sm hover:bg-slate-50"><span
                            class="truncate">{{ empty($filterSales) ? 'Semua Sales' : count($filterSales).' Dipilih' }}</span><i
                            class="fas fa-chevron-down text-[10px] text-slate-400"></i></button>
                    <div x-show="open"
                        class="absolute z-50 mt-1 w-full bg-white border border-slate-200 rounded-lg shadow-xl p-2 max-h-48 overflow-y-auto"
                        style="display: none;">
                        @foreach($optSales as $s) <label
                            class="flex items-center px-2 py-1.5 hover:bg-slate-50 rounded cursor-pointer"><input
                                type="checkbox" value="{{ $s }}" wire:model.live="filterSales"
                                class="rounded border-slate-300 text-indigo-600 mr-2 h-3 w-3"><span
                                class="text-xs text-slate-600">{{ $s }}</span></label> @endforeach
                    </div>
                </div>
                <div wire:loading class="text-indigo-600 text-xs font-bold animate-pulse"><i
                        class="fas fa-spinner fa-spin mr-1"></i></div>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 relative overflow-hidden">
            <h4 class="font-bold text-slate-700 text-lg mb-6 flex items-center gap-2 pb-2 border-b border-slate-100">
                <span class="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center"><i
                        class="fas fa-trophy"></i></span>Top 10 Produk Terlaris (Qty)
            </h4>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 relative overflow-hidden">
            <h4 class="font-bold text-slate-700 text-lg mb-6 flex items-center gap-2 pb-2 border-b border-slate-100">
                <span class="w-8 h-8 rounded-lg bg-purple-50 text-purple-600 flex items-center justify-center"><i
                        class="fas fa-crown"></i></span>Top 10 Pelanggan Terbaik (Value)
            </h4>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 relative overflow-hidden">
            <h4 class="font-bold text-slate-700 text-lg mb-6 flex items-center gap-2 pb-2 border-b border-slate-100">
                <span class="w-8 h-8 rounded-lg bg-pink-50 text-pink-600 flex items-center justify-center"><i
                        class="fas fa-truck"></i></span>Top 10 Supplier (Omzet)
            </h4>
...
document.addEventListener('livewire:init', () => {
    ...
    // --- FUNCTION RENDER OVERVIEW & SALESMAN ---
    ...
    const renderTop = () => {
        const hBarTop = {
            chart: {
                ...
                fontFamily: font,
                // PENTING: Agar chart dirender ulang saat tab visible
    ...
    // Render Top Charts initially but also handle visibility
    // Kita panggil renderTop() segera, tapi karena tab hidden, width-nya 0.
    // Solusinya ada di x-init="$watch..." di atas yang men-trigger resize event.
    renderTop();

    Livewire.on('update-charts', (event) => {
        const data = event.data || event[0].data;
        renderAll(data);
        // Note: Top Ranking datanya tidak ikut ke-update real-time via filter di controller ini
        // Jika ingin ranking ikut filter, controller harus kirim data ranking baru juga.
        // Untuk sekarang kita biarkan static init data.
    });
});
</script>