<div class="space-y-6">
        ...
        <div class="flex flex-col lg:flex-row gap-4 items-end">
            ...
            <div class="w-full lg:w-auto">
                ...
                <div class="flex items-center gap-2">
                    <input type="date" wire:model.live="startDate" class="w-full lg:w-40 text-sm border-gray-200 rounded-lg focus:ring-indigo-500">
                    <span class="text-gray-400 font-bold">-</span>
                    <input type="date" wire:model.live="endDate" class="w-full lg:w-40 text-sm border-gray-200 rounded-lg focus:ring-indigo-500">
                </div>
            </div>
            ...
            <div class="w-full lg:w-64" x-data="{ open: false }">
                ...
                <div class="relative">
                    <button @click="open = !open" @click.outside="open = false" class="w-full bg-white border border-gray-200 text-gray-700 px-3 py-2 rounded-lg text-sm flex items-center justify-between hover:bg-gray-50">
                        <span class="truncate">
                            {{ empty($filterCabang) ? 'Semua Cabang' : count($filterCabang) . ' Cabang Terpilih' }}
                        </span>
                        <i class="fas fa-chevron-down text-xs text-gray-400"></i>
                    </button>
                    <div x-show="open" class="absolute z-50 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-xl p-2 max-h-60 overflow-y-auto">
                        ...
                        <label class="flex items-center px-2 py-1.5 hover:bg-indigo-50 rounded cursor-pointer transition-colors">
                            <input type="checkbox" value="{{ $cab }}" wire:model.live="filterCabang" class="rounded border-gray-300 text-indigo-600 mr-2 focus:ring-indigo-500">
                ...
                <div class="relative">
                    ...
                    <div x-show="open" class="absolute z-50 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-xl p-2 max-h-60 overflow-y-auto">
                        <div class="px-2 pb-2 mb-1 border-b border-gray-100 sticky top-0 bg-white">
                            <input type="text" placeholder="Cari Sales..." class="w-full text-xs border-gray-100 rounded bg-gray-50 focus:ring-0">
                        </div>
                        @foreach($optSales as $sales)
                        <label class="flex items-center px-2 py-1.5 hover:bg-indigo-50 rounded cursor-pointer transition-colors">
                            <input type="checkbox" value="{{ $sales }}" wire:model.live="filterSales" class="rounded border-gray-300 text-indigo-600 mr-2 focus:ring-indigo-500">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="bg-indigo-600 rounded-xl p-5 text-white shadow-lg shadow-indigo-200 transform hover:-translate-y-1 transition-transform duration-300">
            <div class="flex justify-between items-start">
                <div>
                    <p class="text-indigo-200 text-[10px] font-bold uppercase tracking-wider mb-1">Total Penjualan</p>
                    <h3 class="text-2xl font-bold tracking-tight">Rp {{ number_format($salesSum / 1000000, 1, ',', '.') }} Jt</h3>
        ...
        <div class="bg-white rounded-xl p-5 border border-red-100 shadow-sm hover:shadow-md transition-shadow duration-300">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <span class="inline-block mt-2 text-[10px] bg-red-50 text-red-600 px-2 py-0.5 rounded font-bold border border-red-100">
        ...
        <div class="bg-white rounded-xl p-5 border border-orange-100 shadow-sm hover:shadow-md transition-shadow duration-300">
            <div class="flex justify-between items-start">
                <div>
        ...
        <div class="bg-white rounded-xl p-5 border border-emerald-100 shadow-sm hover:shadow-md transition-shadow duration-300">
            <div class="flex justify-between items-start">
                <div>
                    <p class="text-gray-500 text-[10px] font-bold uppercase tracking-wider mb-1">Uang Masuk (Coll)</p>
                    <h3 class="text-xl font-bold text-emerald-600">Rp {{ number_format($collSum / 1000000, 1, ',', '.') }} Jt</h3>
...
<script>
    document.addEventListener('livewire:init', () => {
        ...
        const renderCharts = (data, prodNames, prodQty, custNames, custVal) => {
            ...
            const optSales = {
                series: [
                    { name: 'Penjualan', data: data.sales }, 
                    { name: 'Retur', data: data.retur }
                ],
                ...
                xaxis: { 
                    ...
                    labels: { show: false }, // Sembunyikan label tanggal biar rapi
                    tooltip: { enabled: false }
                }, 
                yaxis: { labels: { formatter: (val) => { return (val/1000000).toFixed(1) + " Jt" }, style: { fontSize: '10px' } } },
                fill: { type: 'gradient', gradient: { opacityFrom: 0.5, opacityTo: 0.05 } },
                tooltip: { y: { formatter: (val) => { return "Rp " + new Intl.NumberFormat('id-ID').format(val) } } },
            ...
            const optAR = {
                series: [
                    { name: 'Tagihan (AR)', data: data.ar }, 
                    { name: 'Bayar (Coll)', data: data.coll }
                ],
            ...
            const optProd = {
                series: [{ name: 'Qty', data: prodQty }],
                chart: { type: 'bar', height: 380, toolbar: { show: false }, fontFamily: 'Plus Jakarta Sans, sans-serif' },
                plotOptions: { bar: { borderRadius: 4, horizontal: true, barHeight: '60%' } },
                dataLabels: { enabled: true, textAnchor: 'start', offsetX: 0, formatter: function (val) { return val } },
                xaxis: { categories: prodNames, labels: { show: false } }, // Sembunyikan sumbu X bawah
                colors: ['#3b82f6'],
                grid: { show: false } // Hilangkan garis grid agar bersih
            };

            // D. TOP CUSTOMER (Bar Horizontal)
            const optCust = {
                series: [{ name: 'Total', data: custVal }],
                chart: { type: 'bar', height: 380, toolbar: { show: false }, fontFamily: 'Plus Jakarta Sans, sans-serif' },
                plotOptions: { bar: { borderRadius: 4, horizontal: true, barHeight: '60%' } },
                dataLabels: { enabled: true, textAnchor: 'start', offsetX: 0, formatter: function (val) { return (val/1000000).toFixed(1) + " Jt" } },
                xaxis: { categories: custNames, labels: { show: false } },
                colors: ['#8b5cf6'],
                grid: { show: false },
            ...
            if(chartSales) {
                ...
                chartProd.destroy(); chartProd = new ApexCharts(document.querySelector("#chart-top-produk"), optProd); chartProd.render();
                chartCust.destroy(); chartCust = new ApexCharts(document.querySelector("#chart-top-customer"), optCust); chartCust.render();
            } else {
                chartSales = new ApexCharts(document.querySelector("#chart-sales-retur"), optSales); chartSales.render();
                chartAR = new ApexCharts(document.querySelector("#chart-ar-coll"), optAR); chartAR.render();
                chartProd = new ApexCharts(document.querySelector("#chart-top-produk"), optProd); chartProd.render();
                chartCust = new ApexCharts(document.querySelector("#chart-top-customer"), optCust); chartCust.render();
            }
        };
