<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="bg-gradient-to-r from-red-500 to-red-600 rounded-xl p-5 text-white shadow-lg relative overflow-hidden">
    ...
    <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
        <div class="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
            ...
            <div class="md:col-span-2">
                ...
                <div class="relative">
                    <input wire:model.live.debounce.500ms="search" type="text" placeholder="No Retur, Pelanggan, Barang..." class="w-full pl-10 text-sm border-gray-200 rounded-lg">
            ...
            <table class="w-full text-xs text-left border-collapse whitespace-nowrap">
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    ...
                    <tr class="hover:bg-red-50 transition-colors">
                        ...
                        <td class="px-4 py-3 text-gray-500">{{ $item->tgl_retur ? date('d/m/Y', strtotime($item->tgl_retur)) : '-' }}</td>
                        <td class="px-4 py-3 font-medium">{{ $item->nama_pelanggan }}</td>
                        <td class="px-4 py-3 text-gray-600 truncate max-w-[200px]" title="{{ $item->nama_item }}">{{ $item->nama_item }}</td>
                        <td class="px-4 py-3 text-center font-bold">{{ $item->qty }}</td>
                        <td class="px-4 py-3 text-right font-mono text-red-600">{{ number_format((float)$item->total_grand, 0, ',', '.') }}</td>
                        <td class="px-4 py-3">
                            <span class="px-2 py-0.5 rounded text-[10px] font-bold bg-gray-100 text-gray-600 border border-gray-200">{{ $item->status }}</span>
                        </td>
                        <td class="px-4 py-3">{{ $item->sales_name }}</td>
                        <td class="px-4 py-3 text-indigo-600 font-bold text-[10px]">{{ $item->cabang }}</td>
                    </tr>
                    @empty
                    <tr><td colspan="9" class="px-6 py-12 text-center text-gray-400">Tidak ada data retur sesuai filter.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
