<div class="space-y-6 font-jakarta">
    ...
    <div class="flex space-x-2 bg-slate-100 p-1.5 rounded-xl w-fit overflow-x-auto">
        <button wire:click="setTab('penjualan')"
            class="px-5 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center gap-2 {{ $activeTab === 'penjualan' ? 'bg-white text-emerald-600 shadow-sm ring-1 ring-slate-200' : 'text-slate-500 hover:text-slate-700' }}">
            <i class="fas fa-chart-line"></i> Penjualan
        </button>
        <button wire:click="setTab('ar')"
            class="px-5 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center gap-2 {{ $activeTab === 'ar' ? 'bg-white text-orange-600 shadow-sm ring-1 ring-slate-200' : 'text-slate-500 hover:text-slate-700' }}">
            <i class="fas fa-money-bill-wave"></i> Piutang
        </button>
        <button wire:click="setTab('supplier')"
            class="px-5 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center gap-2 {{ $activeTab === 'supplier' ? 'bg-white text-purple-600 shadow-sm ring-1 ring-slate-200' : 'text-slate-500 hover:text-slate-700' }}">
            <i class="fas fa-boxes-stacked"></i> Supplier
        </button>
        <button wire:click="setTab('produktifitas')"
            class="px-5 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center gap-2 {{ $activeTab === 'produktifitas' ? 'bg-white text-blue-600 shadow-sm ring-1 ring-slate-200' : 'text-slate-500 hover:text-slate-700' }}">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden min-h-[500px]">
        ...
        @include('livewire.laporan.partials.tab-penjualan') {{-- Atau paste tabel penjualan disini --}}
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="px-6 py-3 align-middle">
                            ...
                            <div class="text-xs font-bold text-center">{{ number_format($row['persen_ims'], 1) }}%</div>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="6" class="p-8 text-center text-slate-400">Tidak ada data.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-6 py-3 text-right font-mono">{{ number_format($row['ar_total'], 0, ',', '.') }}
                        </td>
        ...
        {{-- Paste tabel Supplier --}}
        <div class="overflow-x-auto">
            <table class="w-full text-xs text-left whitespace-nowrap">
                <thead class="bg-purple-50 text-purple-800 font-bold uppercase">
                    <tr>
        ...
        @elseif($activeTab === 'produktifitas')
        {{-- Paste tabel Produktifitas --}}
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                <thead class="bg-blue-50 text-blue-800 font-bold uppercase text-xs">
                    <tr>
                        <th class="px-6 py-4">Nama Sales</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-6 py-3 text-center font-bold text-blue-600 font-mono">{{ $row['real_oa'] }}</td>
