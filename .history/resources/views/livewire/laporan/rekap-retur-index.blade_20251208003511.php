<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div
            class="bg-gradient-to-r from-red-600 to-red-500 rounded-xl p-5 text-white shadow-lg relative overflow-hidden">
        ...
        <div class="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
            ...
            <div class="md:col-span-1">
                ...
                <input type="date" wire:model.live="startDate"
                    class="w-full text-sm border-gray-200 rounded-lg focus:ring-red-500">
            </div>
            <div class="md:col-span-1">
            ...
            <div class="md:col-span-2">
                ...
                <div class="relative">
                    <input wire:model.live.debounce.500ms="search" type="text"
                        ...
                        class="w-full pl-10 text-sm border-gray-200 rounded-lg focus:ring-red-500">
            ...
            <table class="w-full text-xs text-left border-collapse whitespace-nowrap">
                <thead class="text-gray-500 uppercase bg-gray-50 sticky top-0 z-20 shadow-sm font-bold">
                    <tr>
                        ...
                        <th class="px-4 py-3 border-b border-r min-w-[120px]">No Retur</th>
                        <th class="px-4 py-3 border-b border-r min-w-[90px]">Tanggal</th>
                        <th class="px-4 py-3 border-b border-r min-w-[150px]">Pelanggan</th>
                        <th class="px-4 py-3 border-b border-r min-w-[200px]">Barang</th>
                        <th class="px-4 py-3 border-b border-r text-center">Qty</th>
                        <th class="px-4 py-3 border-b border-r text-right bg-red-50 text-red-900">Total Nilai</th>
                        <th class="px-4 py-3 border-b border-r">Status</th>
                        <th class="px-4 py-3 border-b border-r">Sales</th>
                        <th class="px-4 py-3 border-b border-r">No Invoice</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 bg-white">
                    @forelse ($retur as $item)
                    <tr class="hover:bg-red-50 transition-colors">
                        <td class="px-4 py-2 border-r text-red-600 font-bold text-[10px]">{{ $item->cabang }}</td>
                        <td class="px-4 py-2 border-r font-bold text-gray-700">{{ $item->no_retur }}</td>
                        <td class="px-4 py-2 border-r text-gray-500">
                        ...
                        <td class="px-4 py-2 border-r text-gray-600 truncate max-w-[200px]"
                            title="{{ $item->nama_item }}">{{ $item->nama_item }}</td>
                        <td class="px-4 py-2 border-r text-center font-bold">{{ $item->qty }}</td>
                        <td class="px-4 py-2 border-r text-right font-mono font-bold text-red-600">
                        ...
                        <td class="px-4 py-2 border-r text-xs text-gray-400">{{ $item->no_inv }}</td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="10" class="px-6 py-12 text-center text-gray-400">Tidak ada data retur sesuai
                            filter.</td>
                    </tr>
                    @endforelse
                </tbody>
