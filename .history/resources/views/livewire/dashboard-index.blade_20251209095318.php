<div class="min-h-screen space-y-6 font-jakarta pb-10" x-data="{ activeTab: 'overview' }" x-init="$watch('activeTab', value => {
         if (value === 'ranking') {
             // Paksa resize window agar chart.js menggambar ulang di tab tersembunyi
    ...
    <div x-show="activeTab === 'overview'" x-transition class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 px-1">
            ...
            <div class="bg-white p-5 rounded-2xl border border-emerald-100 shadow-sm hover:shadow-md transition-shadow">
                <p class="text-slate-400 text-[10px] font-bold uppercase tracking-wider">Collection</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                ...
                <div class="relative h-72 w-full"><canvas id="salesReturChart"></canvas></div>
            </div>
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                <h4 class="font-bold text-slate-800 text-lg mb-4">💰 Tagihan vs Pembayaran</h4>
                <div class="relative h-72 w-full"><canvas id="arCollectionChart"></canvas></div>
            </div>
        </div>
    </div>

    <div x-show="activeTab === 'ranking'" x-transition class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 relative overflow-hidden">
            <h4 class="font-bold text-slate-700 text-lg mb-6 flex items-center gap-2 pb-2 border-b border-slate-100">
                <span class="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center"><i
                        class="fas fa-trophy"></i></span>Top 10 Produk (Qty)
            </h4>
            <div class="relative h-80 w-full"><canvas id="topProductChart"></canvas></div>
        </div>
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 relative overflow-hidden">
            <h4 class="font-bold text-slate-700 text-lg mb-6 flex items-center gap-2 pb-2 border-b border-slate-100">
                <span class="w-8 h-8 rounded-lg bg-purple-50 text-purple-600 flex items-center justify-center"><i
                        class="fas fa-crown"></i></span>Top 10 Pelanggan (Omzet)
            </h4>
            <div class="relative h-80 w-full"><canvas id="topCustomerChart"></canvas></div>
        </div>
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 relative overflow-hidden">
            <h4 class="font-bold text-slate-700 text-lg mb-6 flex items-center gap-2 pb-2 border-b border-slate-100">
            ...
            <div class="relative h-80 w-full"><canvas id="topSupplierChart"></canvas></div>
        </div>
    </div>

    <div x-show="activeTab === 'salesman'" x-transition class="grid grid-cols-1 gap-6">
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            <h4 class="font-bold text-lg text-indigo-900 mb-4 pb-2 border-b border-slate-50">🎯 Sales Performance (Top
                20)</h4>
            <div class="relative h-96 w-full"><canvas id="imsChart"></canvas></div>
        </div>
    </div>
...
document.addEventListener('livewire:init', () => {
    ...
    let charts = {};
    const initData = @json($chartData); // Ambil semua data chart dari sini
    ...
    const renderCharts = (data) => {
        ...
        charts.salesRetur = new Chart(document.getElementById('salesReturChart'), {
            type: 'line',
            data: {
                labels: data.dates,
                datasets: [{
        ...
        charts.arColl = new Chart(document.getElementById('arCollectionChart'), {
            type: 'bar',
            data: {
                labels: data.dates,
                datasets: [{
        ...
        // --- RANKING (DIAMBIL DARI data.topProdNames, dst) ---
        ...
        charts.topProd = new Chart(document.getElementById('topProductChart'), {
            type: 'bar',
            data: {
                labels: data.topProdNames, // <--- Perhatikan ini
                datasets: [{
                    label: 'Qty',
