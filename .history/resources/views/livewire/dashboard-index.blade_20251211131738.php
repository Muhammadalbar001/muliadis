<div class="min-h-screen space-y-6 font-jakarta pb-10" x-data="{ activeTab: 'overview' }">
    ...
    <div x-show="activeTab === 'ranking'" x-transition.opacity.duration.300ms
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            ...
            <div id="chart-top-supplier" style="min-height: 350px;"></div>
        </div>
    </div>

    <div x-show="activeTab === 'salesman'" x-transition.opacity.duration.300ms class="grid grid-cols-1" wire:ignore>
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            ...
            <div id="chart-sales-perf" style="min-height: 400px;"></div>
        </div>
    </div>
...
document.addEventListener('livewire:init', () => {
    let charts = {};
    const initData = @json($chartData); // Data awal dari PHP

    // Fungsi Render Semua Chart
    const renderCharts = (data) => {
        ...
        // Template Horizontal Bar untuk Ranking
        const rankingOpts = {
            chart: {
                type: 'bar',
                height: 350,
        ...
        // 6. Sales Performance (Target vs Real)
    ...
    // Render Ulang saat Filter Berubah (Event Listener)
    Livewire.on('update-charts', (event) => {
        // Ambil data payload dengan benar (Livewire 3 terkadang membungkus dalam array)
