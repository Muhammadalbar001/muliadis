            ...
            <div class="flex flex-wrap sm:flex-nowrap gap-2 items-center w-full xl:w-auto justify-end">
                ...
                <div class="relative w-full sm:w-48" x-data="{ 
                    ...
                    <button @click="open = !open; $nextTick(() => $refs.searchInput.focus())"
                        ...
                        <span class="truncate"
                            x-text="selected.length > 0 ? selected.length + ' Dipilih' : 'Semua Supplier'"></span>
                        <i class="fas fa-chevron-down text-[10px] text-slate-400 transition-transform"
                            :class="{'rotate-180': open}"></i>
                    </button>
                    ...
                    <div x-show="open" x-transition
                        ...
                        <div class="max-h-48 overflow-y-auto custom-scrollbar space-y-1">
                            ...
                            <div x-show="'{{ strtolower($s) }}'.includes(search.toLowerCase())" @click="
                                ...
                                "
                                class="px-2 py-1.5 rounded-md cursor-pointer text-xs font-medium hover:bg-emerald-50 hover:text-emerald-700 flex items-center justify-between transition-colors group">

                                <div class="flex items-center gap-2 overflow-hidden">
                                    <div class="w-4 h-4 rounded border flex items-center justify-center transition-colors"
                                        :class="selected.includes('{{ $s }}') ? 'bg-emerald-500 border-emerald-500' : 'border-slate-300 bg-white group-hover:border-emerald-400'">
                                        <i x-show="selected.includes('{{ $s }}')"
                                            class="fas fa-check text-white text-[9px]"></i>
                                    </div>
                                    <span class="truncate"
                                        :class="selected.includes('{{ $s }}') ? 'text-emerald-700 font-bold' : 'text-slate-600'">{{ $s }}</span>
                                </div>
                            </div>
                            @endforeach
                        ...
                        <div class="pt-2 border-t border-slate-100 mt-1 text-[10px] text-slate-400 text-center">
                            <span x-text="selected.length"></span> supplier dipilih
                        </div>
                    </div>
                </div>
