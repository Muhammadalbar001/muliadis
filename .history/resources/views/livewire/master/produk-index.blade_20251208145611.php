            ...
            <table class="w-full text-sm text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors group">
                        <td class="px-6 py-3 text-center text-slate-400 text-xs">{{ $produk->firstItem() + $index }}
                        </td>
                        <td class="px-6 py-3 font-mono text-xs text-indigo-600 font-bold">{{ $item->kode_item }}</td>
                        <td class="px-6 py-3 font-bold text-slate-700">{{ $item->nama_item }}</td>
                        ...
                        <td class="px-6 py-3 text-center">
                            ...
                            $persen = $item->harga_beli > 0 ? ($margin / $item->harga_beli) * 100 : 0;
                            @endphp
            ...
            <div
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            <h3 class="text-lg leading-6 font-bold text-slate-900" id="modal-title">Import Data Produk
                            </h3>
                            <div class="mt-2">
                                <p class="text-sm text-slate-500 mb-4">
                                    Upload file Excel (.xlsx) yang berisi daftar produk baru. <br>
                                    <span class="text-xs text-indigo-500">*Pastikan header kolom sesuai format.</span>
                                </p>

                                <div
                                    class="mt-2 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-xl hover:bg-slate-50 hover:border-indigo-400 cursor-pointer transition-colors group relative">
                                    <div class="space-y-1 text-center">
                                        <i
                                            class="fas fa-cloud-upload-alt text-4xl text-slate-400 group-hover:text-indigo-500 transition-colors"></i>
                                        <div class="flex text-sm text-slate-600 justify-center mt-2">
                                            <label for="file-upload-produk"
                                                class="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none">
                                                <span>Pilih File</span>
                                                <input id="file-upload-produk" wire:model="file" type="file"
                                                    class="sr-only">
                                            </label>
                                        </div>
                                        <p class="text-xs text-slate-400">XLSX, CSV up to 10MB</p>
                                    </div>
                                ...
                                <div class="mt-4 flex items-center p-3 bg-red-50 rounded-lg border border-red-100">
                                    ...
                                    <label for="resetData" class="ml-3 block text-sm text-red-800 cursor-pointer">
                                        <span class="font-bold">Kosongkan Data Lama?</span>
                                        <span class="block text-xs text-red-600 mt-0.5">Centang ini jika ingin mengganti
                                            seluruh data produk.</span>
                                    </label>
                                </div>

                                <div wire:loading wire:target="file" class="w-full mt-2 text-center">
                                    <span class="text-xs text-indigo-600 font-bold animate-pulse">Sedang
                                        mengupload...</span>
                                </div>
                                ...
                                @error('file')
                                <div class="mt-2 text-red-500 text-xs flex items-center"><i
                ...
                <div class="bg-slate-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
                    <button wire:click="import" wire:loading.attr="disabled" type="button"
                        class="w-full inline-flex justify-center rounded-xl border border-transparent shadow-sm px-4 py-2 bg-indigo-600 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none sm:ml-3 sm:w-auto sm:text-sm disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                        Mulai Import
                    </button>
                    <button wire:click="closeImportModal" type="button"
                        class="mt-3 w-full inline-flex justify-center rounded-xl border border-slate-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-slate-700 hover:bg-slate-50 focus:outline-none sm:mt-0 sm:w-auto sm:text-sm transition-colors">
                        Batal
                    </button>
                </div>
            </div>
