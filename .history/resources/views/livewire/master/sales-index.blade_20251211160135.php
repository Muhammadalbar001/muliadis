<div class="space-y-6 font-jakarta">
    ...
    <div
        ...
        <div class="flex flex-col xl:flex-row gap-4 items-center justify-between">
            ...
            <div class="flex items-center gap-4 w-full xl:w-auto">
                <div>
                    <h1 class="text-xl font-extrabold text-slate-800 tracking-tight">Master Salesman</h1>
                    <p class="text-xs text-slate-500 mt-0.5">Kelola data tim penjualan, divisi, & target.</p>
                </div>
                <div
                    class="hidden md:flex px-3 py-1 bg-indigo-50 text-indigo-600 rounded-lg text-[10px] font-bold border border-indigo-100 items-center gap-2">
            ...
            <div class="flex flex-wrap sm:flex-nowrap gap-2 items-center w-full xl:w-auto justify-end">
                ...
                <div class="relative w-full sm:w-48">
                    ...
                    <input wire:model.live.debounce.300ms="search" type="text"
                        class="pl-8 w-full border-slate-200 rounded-lg text-xs font-bold text-slate-700 focus:ring-indigo-500 py-2 shadow-sm placeholder-slate-400"
                        placeholder="Cari Sales...">
                </div>

                <div class="w-full sm:w-36">
                    <select wire:model.live="filterCity"
                        class="w-full border-slate-200 rounded-lg text-xs font-bold text-slate-700 focus:ring-indigo-500 py-2 shadow-sm cursor-pointer">
                ...
                <button wire:click="resetAllTargets"
                    ...
                    title="Reset Target">
                    <i class="fas fa-eraser"></i> <span class="hidden sm:inline">Reset</span>
                </button>

                <button wire:click="openImportModal"
                    class="px-3 py-2 bg-indigo-600 text-white rounded-lg text-xs font-bold hover:bg-indigo-700 shadow-md shadow-indigo-500/20 transition-all flex items-center gap-2">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors group">
                        ...
                        <td class="px-6 py-4 text-center">
                            ...
                            @else
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-500 border border-slate-200">
                        ...
                        <td class="px-6 py-4 text-center">
                            <button wire:click="openTargetModal({{ $item->id }})"
                                ...
                                <i class="fas fa-bullseye mr-1.5 group-hover/btn:scale-110 transition-transform"></i>
                                Set Target
                            </button>
                        </td>
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-slate-500 text-sm mt-1 mb-4">Belum ada data sales yang sesuai dengan
                                    filter Anda.</p>
                                <button wire:click="openImportModal"
    ...
    <div class="fixed inset-0 z-[60] overflow-y-auto" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div
                class="relative inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md w-full border border-white/20">
                ...
                <div class="px-6 py-6">
                    <div class="mb-4">
                        <div
                            class="w-full flex justify-center px-6 pt-8 pb-8 border-2 border-slate-300 border-dashed rounded-xl hover:bg-slate-50 cursor-pointer relative transition-all group hover:border-indigo-400 bg-slate-50/50">
                            <div class="space-y-2 text-center">
                                ...
                                <div class="text-sm text-slate-600">
                                    <label for="file-upload-sales"
                                        class="relative cursor-pointer rounded-md font-bold text-indigo-600 hover:text-indigo-500 focus-within:outline-none">
                    ...
                    <div class="bg-rose-50 border border-rose-100 rounded-xl p-4 mb-4">
                        <label class="flex items-start gap-3 cursor-pointer">
                            ...
                            <div>
                                ...
                                <span class="block text-xs text-rose-600 mt-0.5">Data lama akan dihapus total.</span>
                            </div>
                        </label>
                    </div>
                    ...
                    @error('file')
                    <div
                        class="p-3 bg-red-50 border border-red-100 text-red-600 text-xs rounded-lg flex items-center gap-2">
                ...
                <div class="bg-slate-50 px-6 py-4 flex flex-row-reverse gap-3 border-t border-slate-200">
                    <button wire:click="import" wire:loading.attr="disabled"
                        class="w-full inline-flex justify-center items-center rounded-xl px-4 py-2.5 bg-indigo-600 text-sm font-bold text-white hover:bg-indigo-700 focus:outline-none shadow-lg shadow-indigo-500/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed">Mulai
                        Import</button>
                    <button wire:click="closeImportModal"
                        class="w-full inline-flex justify-center items-center rounded-xl border border-slate-300 px-4 py-2.5 bg-white text-sm font-bold text-slate-700 hover:bg-slate-50 focus:outline-none transition-all">Batal</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="fixed inset-0 z-[60] overflow-y-auto" role="dialog" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div
                class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-3xl w-full border border-white/20">
                <div
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-white flex items-center gap-2"><i class="fas fa-bullseye"></i>
                            Set Target Sales</h3>
                        <p class="text-blue-100 text-xs mt-0.5">Salesman: <span
                ...
                <div class="px-6 py-6 bg-slate-50 max-h-[65vh] overflow-y-auto custom-scrollbar">
                    <div
                        class="grid grid-cols-12 gap-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 px-2">
                    ...
                    <div class="space-y-2.5">
                        ...
                        <div
                            class="grid grid-cols-12 gap-4 items-center bg-white p-3 rounded-xl border border-slate-200 shadow-sm hover:border-indigo-300 hover:shadow-md transition-all group">
                            <div class="col-span-2 font-bold text-slate-700 text-sm flex items-center gap-3">
                                <span
                                    class="w-8 h-8 rounded-lg bg-slate-100 text-slate-600 flex items-center justify-center text-xs border border-slate-200 group-hover:bg-indigo-50 group-hover:text-indigo-600 group-hover:border-indigo-100 transition-colors">{{ $m }}</span>
                                <span
                            ...
                            <div class="col-span-5">
                                <div class="relative">
                                    <span
                                        class="absolute left-3 top-2.5 text-slate-400 text-xs font-bold pointer-events-none group-focus-within:text-indigo-500">Rp</span>
                            ...
                            <div class="col-span-5">
                                <div class="relative">
                                    <span
                                        class="absolute left-3 top-2.5 text-slate-400 text-xs pointer-events-none group-focus-within:text-indigo-500"><i
                ...
                <div class="bg-white px-6 py-4 border-t border-slate-200 flex justify-end gap-3">
                    ...
                    <button wire:click="saveTargets"
                        class="px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-bold hover:bg-indigo-700 focus:outline-none shadow-lg shadow-indigo-500/30 flex items-center gap-2 transition-all transform hover:-translate-y-0.5"><i
