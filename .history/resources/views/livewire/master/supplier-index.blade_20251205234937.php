    ...
    <div
        ...
        <div class="relative w-full md:w-96">
            ...
            <input wire:model.live.debounce.300ms="search" type="text"
                ...
                placeholder="Cari nama supplier, kode, atau kota...">
        </div>

        <button wire:click="create"
            class="inline-flex items-center px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-all shadow-sm hover:shadow-md">
            ...
            <table class="w-full text-sm text-left">
                <thead
                    ...
                    <tr>
                        ...
                        <th class="px-6 py-4">Alamat / Kota</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors group">
                        ...
                        <td class="px-6 py-4 font-mono text-blue-600">{{ $item->kode_supplier }}</td>
                        <td class="px-6 py-4 font-medium text-gray-800">{{ $item->nama_supplier }}</td>
                        <td class="px-6 py-4 text-gray-600">
                            <span class="block truncate max-w-xs">{{ $item->alamat ?? '-' }}</span>
                            <span class="text-xs text-gray-400">{{ $item->kota ?? '' }}</span>
                        </td>
                        <td class="px-6 py-4 text-gray-600">
                            @if($item->telepon)
                            <div class="flex items-center gap-1"><i class="fas fa-phone text-xs text-gray-400"></i>
                                {{ $item->telepon }}</div>
                            @else
            ...
            <div
                ...
                <div class="px-6 py-4 space-y-4">
                    <div>
                        ...
                        <input type="text" wire:model="kode_supplier"
                            class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        @error('kode_supplier') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <textarea wire:model="alamat" rows="2"
                            class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 sm:text-sm"></textarea>
                    </div>
                </div>
