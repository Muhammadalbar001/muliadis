<div class="space-y-6 font-jakarta">
    ...
    <div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-end gap-4">
            ...
            <table class="w-full text-sm text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="px-6 py-3 text-center text-slate-400 text-xs">{{ $suppliers->firstItem() + $index }}</td>
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="relative inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
                    ...
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            ...
                            <div class="mt-2">
                                ...
                                <div class="mt-2 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-xl hover:bg-pink-50 hover:border-pink-400 cursor-pointer transition-colors group relative">
                                    <div class="space-y-1 text-center">
                                        <i class="fas fa-cloud-upload-alt text-4xl text-slate-400 group-hover:text-pink-500 transition-colors"></i>
                                        <div class="flex text-sm text-slate-600 justify-center mt-2">
                                            <label for="file-upload-supp" class="relative cursor-pointer bg-white rounded-md font-medium text-pink-600 hover:text-pink-500 focus-within:outline-none">
                                                <span>Pilih File</span>
                                                <input id="file-upload-supp" wire:model="file" type="file" class="sr-only">
                                            </label>
                                        </div>
                                    </div>
                                    @if($file)
                                        <div class="absolute inset-0 bg-pink-50 bg-opacity-90 flex flex-col items-center justify-center rounded-xl">
                                            <i class="fas fa-check text-pink-500 text-2xl mb-1"></i>
                                            <span class="text-xs font-bold text-slate-700">{{ $file->getClientOriginalName() }}</span>
                                        </div>
                                    @endif
                                </div>

                                <div class="mt-4 flex items-center p-3 bg-red-50 rounded-lg border border-red-100">
                                    <input id="resetData" type="checkbox" wire:model="resetData" class="h-4 w-4 text-red-600 focus:ring-red-500 border-slate-300 rounded cursor-pointer">
                                    <label for="resetData" class="ml-3 block text-sm text-red-800 cursor-pointer">
                                        <span class="font-bold">Reset Data?</span>
                                        <span class="text-xs block text-red-600">Hapus semua data supplier lama sebelum import.</span>
                                    </label>
                                </div>
                                
                                <div wire:loading wire:target="import" class="w-full mt-2 text-center text-xs text-pink-600 font-bold animate-pulse">Proses Import...</div>
                ...
                <div class="bg-slate-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
                    <button wire:click="import" wire:loading.attr="disabled" type="button" class="w-full inline-flex justify-center rounded-xl border border-transparent shadow-sm px-4 py-2 bg-pink-600 text-base font-medium text-white hover:bg-pink-700 sm:ml-3 sm:w-auto sm:text-sm">Import</button>
                    <button wire:click="closeImportModal" type="button" class="mt-3 w-full inline-flex justify-center rounded-xl border border-slate-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-slate-700 hover:bg-slate-50 sm:mt-0 sm:w-auto sm:text-sm">Batal</button>
                </div>
            </div>
        </div>
    </div>
