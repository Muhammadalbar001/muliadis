                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-blue-50 transition duration-150">
                        ...
                        <td class="px-2 py-1 border-r text-right bg-blue-50 font-mono whitespace-nowrap">Rp
                            {{ number_format($item->buy, 2, ',', '.') }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">
                            {{ number_format($item->buy_disc, 2, ',', '.') }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">
                            {{ number_format($item->buy_in_ktn, 2, ',', '.') }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">
                            {{ number_format($item->avg, 2, ',', '.') }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">
                            {{ number_format($item->total, 2, ',', '.') }}</td>

                        <!-- === MARGIN & ORDER (42 - 48) === -->
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">
                            {{ number_format($item->up, 2, ',', '.') }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">
                            {{ number_format($item->fix, 2, ',', '.') }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">
                            {{ number_format($item->ppn, 2, ',', '.') }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">
                            {{ number_format($item->fix_exc_ppn, 2, ',', '.') }}</td>

                        <!-- MARGIN (46) - Tampilkan kurung jika minus -->
                        <td
                            class="px-2 py-1 border-r text-right whitespace-nowrap @if($item->margin < 0) text-red-600 font-bold @endif">
                            @if($item->margin < 0) ({{ number_format(abs($item->margin), 2, ',', '.') }}) @else
                                {{ number_format($item->margin, 2, ',', '.') }} @endif </td>

                                <!-- % MARGIN (47) - Tampilkan kurung jika minus -->
                        <td
                            class="px-2 py-1 border-r text-right whitespace-nowrap @if($item->percent_margin < 0) text-red-600 font-bold @endif">
                            @if($item->percent_margin < 0)
                                ({{ number_format(abs($item->percent_margin), 2, ',', '.') }})% @else
                                {{ number_format($item->percent_margin, 2, ',', '.') }}% @endif </td>

                                <!-- ORDER NO (String) - REVISI NAMA KOLOM -->
