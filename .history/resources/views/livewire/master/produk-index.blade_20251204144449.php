<div class="space-y-4">
    ...
    <div class="bg-white border border-gray-300 shadow-sm rounded-lg flex flex-col"> 
        <!-- Kontainer Scroll Utama: overflow-x-auto untuk horizontal, overflow-y-auto untuk vertikal -->
        <div class="flex-1 overflow-x-auto overflow-y-auto" style="max-height: calc(100vh - 200px);"> 
            <table class="min-w-max text-xs text-left border-collapse table-auto">
                ...
                <thead class="text-xs text-gray-700 uppercase bg-gray-100 sticky top-0 z-20">
                    <tr>
                        ...
                        <!-- Sticky left: 0px (Cabang) -->
                        <th class="px-2 py-2 border-b border-r bg-gray-100 sticky left-0 z-30 min-w-[80px] whitespace-nowrap">Cabang</th>
                        <!-- Sticky left: 80px (SKU) -->
                        <th class="px-2 py-2 border-b border-r bg-gray-100 sticky left-[80px] z-30 min-w-[100px] whitespace-nowrap">SKU</th>
                        <!-- Sticky left: 180px (Nama Item) -->
                        <th class="px-2 py-2 border-b border-r bg-gray-100 sticky left-[180px] z-30 min-w-[200px] whitespace-nowrap">Nama Item</th>
                        ...
                        <th class="px-2 py-2 border-b border-r min-w-[60px] text-center font-bold bg-yellow-50 whitespace-nowrap">Stok</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-blue-50 transition duration-150">
                        <!-- === Kolom Fixed Kiri === -->
                        <td class="px-2 py-1 border-r bg-white sticky left-0 z-10 font-bold text-blue-800 whitespace-nowrap">{{ $item->cabang }}</td>
                        <td class="px-2 py-1 border-r bg-white sticky left-[80px] z-10 font-mono whitespace-nowrap">{{ $item->sku }}</td>
                        <td class="px-2 py-1 border-r bg-white sticky left-[180px] z-10 text-gray-900 truncate max-w-[200px] whitespace-nowrap" title="{{ $item->name_item }}">
                            {{ $item->name_item }}
                        </td>
                        ...
                        <td class="px-2 py-1 border-r whitespace-nowrap {{ $item->expired_date && \Carbon\Carbon::parse($item->expired_date)->isPast() ? 'text-red-600 font-bold' : '' }}">
                            {{ $item->expired_date ? \Carbon\Carbon::parse($item->expired_date)->format('d-m-Y') : '-' }}
                        </td>
                        <td class="px-2 py-1 border-r text-center font-bold bg-yellow-50 whitespace-nowrap">{{ number_format($item->stok, 0) }}</td>
                        <td class="px-2 py-1 border-r text-center whitespace-nowrap">{{ $item->oum }}</td>

                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->good, 0) }}</td>
                        <td class="px-2 py-1 border-r text-center whitespace-nowrap">{{ $item->good_konversi }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->ktn, 2) }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->good_amount, 2) }}</td>

                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->avg_3m_in_oum, 2) }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->avg_3m_in_ktn, 2) }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->avg_3m_in_value, 2) }}</td>
                        <td class="px-2 py-1 border-r text-center whitespace-nowrap">{{ $item->not_move_3m }}</td>

                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->bad, 0) }}</td>
                        <td class="px-2 py-1 border-r text-center whitespace-nowrap">{{ $item->bad_konversi }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->bad_ktn, 2) }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->bad_amount, 2) }}</td>

                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->wrh1, 0) }}</td>
                        <td class="px-2 py-1 border-r text-center whitespace-nowrap">{{ $item->wrh1_konversi }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->wrh1_amount, 2) }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->wrh2, 0) }}</td>
                        <td class="px-2 py-1 border-r text-center whitespace-nowrap">{{ $item->wrh2_konversi }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->wrh2_amount, 2) }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->wrh3, 0) }}</td>
                        <td class="px-2 py-1 border-r text-center whitespace-nowrap">{{ $item->wrh3_konversi }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->wrh3_amount, 2) }}</td>

                        <td class="px-2 py-1 border-r whitespace-nowrap">{{ $item->good_storage }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->sell_per_week, 2) }}</td>
                        ...
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->min, 0) }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->re_qty, 0) }}</td>
                        <td class="px-2 py-1 border-r text-center whitespace-nowrap">{{ $item->expired_info ? \Carbon\Carbon::parse($item->expired_info)->format('d-m-Y') : '-' }}</td>

                        <td class="px-2 py-1 border-r text-right bg-blue-50 font-mono whitespace-nowrap">Rp {{ number_format($item->buy, 0) }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->buy_disc, 0) }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->buy_in_ktn, 0) }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->avg, 0) }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->total, 0) }}</td>

                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->up, 0) }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->fix, 0) }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->ppn, 0) }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->fix_exc_ppn, 0) }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->margin, 0) }}</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->percent_margin, 2) }}%</td>
                        <td class="px-2 py-1 border-r text-right whitespace-nowrap">{{ number_format($item->order_qty, 0) }}</td>

                        <td class="px-2 py-1 border-r truncate max-w-[150px] whitespace-nowrap" title="{{ $item->supplier }}">{{ $item->supplier }}</td>
                        <td class="px-2 py-1 border-r whitespace-nowrap">{{ $item->mother_sku }}</td>
                        <td class="px-2 py-1 border-r truncate max-w-[150px] whitespace-nowrap" title="{{ $item->last_supplier }}">{{ $item->last_supplier }}</td>
                        <td class="px-2 py-1 border-r whitespace-nowrap">{{ $item->divisi }}</td>
                        <td class="px-2 py-1 border-r text-xs text-gray-400 whitespace-nowrap">{{ $item->unique_id }}</td>
                    </tr>
                    @empty
                    <tr>
