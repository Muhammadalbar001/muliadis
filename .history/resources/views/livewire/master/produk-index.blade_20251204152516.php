<div class="space-y-4">
    <!-- 
        INI ADALAH SATU-SATUNYA ROOT ELEMEN UTAMA.
        SEMUA KONTEN LAIN HARUS ADA DI DALAM DIV INI.
    -->

    <!-- AREA NOTIFIKASI -->
    <div>
        <!-- Cek Tag @if tidak boleh lepas dari div -->
            ...
            <table class="min-w-max text-xs text-left border-collapse table-auto">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-blue-50 transition duration-150">
                        ...
                        <!-- MARGIN (46) - Tampilkan kurung jika ada minus di data string -->
                        <td
                            class="px-2 py-1 border-r text-right whitespace-nowrap @if(str_contains($item->margin, '-')) text-red-600 font-bold @endif">
                            @php
                            $margin = str_replace('-', '', $item->margin);
                            @endphp
                            @if(str_contains($item->margin, '-'))
                            ({{ $margin }})
                            @else
                            {{ $item->margin }}
                            @endif
                        ...
                        <!-- % MARGIN (47) - Tampilkan kurung jika ada minus di data string -->
                        <td
                            class="px-2 py-1 border-r text-right whitespace-nowrap @if(str_contains($item->percent_margin, '-')) text-red-600 font-bold @endif">
                            @php
                            $percentMargin = str_replace('-', '', $item->percent_margin);
                            @endphp
                            @if(str_contains($item->percent_margin, '-'))
                            ({{ $percentMargin }})%
                            @else
                            {{ $item->percent_margin }}%
                            @endif
