<div class="space-y-6 font-jakarta">
    ...
    <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            <div>
                ...
                <select wire:model.live="filterCabang" class="w-full border-slate-200 rounded-lg text-xs focus:ring-indigo-500 text-slate-700">
            ...
            <div>
                ...
                <select wire:model.live="filterSupplier" class="w-full border-slate-200 rounded-lg text-xs focus:ring-indigo-500 text-slate-700">
            ...
            <div>
                ...
                <select wire:model.live="filterKategori" class="w-full border-slate-200 rounded-lg text-xs focus:ring-indigo-500 text-slate-700">
            ...
            <div>
                ...
                <select wire:model.live="filterDivisi" class="w-full border-slate-200 rounded-lg text-xs focus:ring-indigo-500 text-slate-700">
            ...
            <div>
                ...
                <select wire:model.live="filterStok" class="w-full border-slate-200 rounded-lg text-xs focus:ring-indigo-500 text-slate-700">
            ...
            <div class="flex items-end">
                <button wire:click="resetFilter" class="w-full py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-bold rounded-lg transition-colors flex items-center justify-center gap-2 border border-slate-200">
            ...
            <table class="text-xs text-left border-collapse whitespace-nowrap min-w-max">
                ...
                <thead class="text-slate-500 uppercase bg-slate-50 font-bold border-b border-slate-200 sticky top-0 z-20 shadow-sm">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-center bg-slate-100 border-l sticky right-0 z-30 w-24 shadow-sm">Aksi</th>
                    </tr>
                </thead>
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    ...
                    <tr class="hover:bg-indigo-50 transition-colors {{ $item->is_duplicate ? 'bg-red-50' : '' }}">
                        ...
                        <td class="px-4 py-2 border-r font-bold text-slate-700 bg-slate-50/30 truncate max-w-[300px]" title="{{ $item->name_item }}">
                            {{ $item->name_item }}
                            @if($item->is_duplicate) <span class="ml-1 text-[9px] bg-red-100 text-red-600 px-1 rounded">DUP</span> @endif
                        </td>
                        <td class="px-4 py-2 border-r">{{ $item->expired_date }}</td>

                        <td class="px-4 py-2 border-r text-right font-bold {{ $item->stok <= 0 ? 'text-red-500' : 'text-emerald-600' }}">
                        ...
                        <td class="px-4 py-2 border-r text-right">{{ number_format((float)$item->good_amount, 0, ',', '.') }}</td>

                        <td class="px-4 py-2 border-r text-right">{{ $item->avg_3m_in_oum }}</td>
                        <td class="px-4 py-2 border-r text-right">{{ $item->avg_3m_in_ktn }}</td>
                        <td class="px-4 py-2 border-r text-right">{{ number_format((float)$item->avg_3m_in_value, 0, ',', '.') }}</td>
                        ...
                        <td class="px-4 py-2 border-r text-right">{{ number_format((float)$item->bad_amount, 0, ',', '.') }}</td>
                        ...
                        <td class="px-4 py-2 border-r text-right">{{ number_format((float)$item->buy, 0, ',', '.') }}</td>
                        <td class="px-4 py-2 border-r text-right">{{ number_format((float)$item->buy_disc, 0, ',', '.') }}</td>
                        <td class="px-4 py-2 border-r text-right">{{ number_format((float)$item->buy_in_ktn, 0, ',', '.') }}</td>
                        <td class="px-4 py-2 border-r text-right">{{ number_format((float)$item->avg, 0, ',', '.') }}</td>
                        <td class="px-4 py-2 border-r text-right font-medium">{{ number_format((float)$item->total, 0, ',', '.') }}</td>

                        <td class="px-4 py-2 border-r text-right">{{ number_format((float)$item->up, 0, ',', '.') }}</td>
                        <td class="px-4 py-2 border-r text-right">{{ number_format((float)$item->fix, 0, ',', '.') }}</td>
                        <td class="px-4 py-2 border-r text-right">{{ number_format((float)$item->ppn, 0, ',', '.') }}</td>
                        <td class="px-4 py-2 border-r text-right">{{ number_format((float)$item->fix_exc_ppn, 0, ',', '.') }}</td>
                        <td class="px-4 py-2 border-r text-right font-bold text-emerald-600">{{ number_format((float)$item->margin, 0, ',', '.') }}</td>
                        <td class="px-4 py-2 border-r text-right">{{ number_format((float)$item->percent_margin, 2, ',', '.') }}%</td>
                        ...
                        <td class="px-4 py-2 text-center sticky right-0 bg-white border-l z-10 group-hover:bg-indigo-50">
    ...
    <div class="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-slate-900 bg-opacity-75 transition-opacity backdrop-blur-sm" wire:click="closeImportModal"></div>
            <div class="relative inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div class="flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-xl hover:bg-slate-50 cursor-pointer relative">
                    ...
                    <div wire:loading wire:target="file" class="text-center mt-2 text-xs text-indigo-600 font-bold">Mengupload...</div>
                    @if($file) <p class="mt-2 text-center text-sm font-bold text-emerald-600">{{ $file->getClientOriginalName() }}</p> @endif
                </div>
                <div class="bg-slate-50 px-4 py-3 sm:flex sm:flex-row-reverse gap-2">
                    <button wire:click="import" wire:loading.attr="disabled" class="w-full sm:w-auto bg-indigo-600 text-white px-4 py-2 rounded-lg font-bold hover:bg-indigo-700">Import Sekarang</button>
                    <button wire:click="closeImportModal" class="w-full sm:w-auto bg-white border border-slate-300 text-slate-700 px-4 py-2 rounded-lg hover:bg-slate-50">Batal</button>
                </div>
            </div>
        </div>
    </div>
