<div class="flex flex-col h-[calc(100vh-6rem)] gap-4 font-jakarta">

    <div
        class="flex-none sticky top-0 z-40 backdrop-blur-md bg-white/90 p-4 rounded-2xl shadow-sm border border-slate-200">
        <div class="flex flex-col xl:flex-row gap-4 items-center justify-between">

            <div class="flex items-center gap-4 w-full xl:w-auto">
                <div>
                    <h1 class="text-xl font-extrabold text-slate-800 tracking-tight">Master Produk</h1>
                    <p class="text-xs text-slate-500 mt-0.5">{{ $produks->total() }} SKU Terdaftar</p>
                </div>
            </div>

            <div
                class="flex flex-wrap sm:flex-nowrap gap-2 items-center w-full xl:w-auto justify-end overflow-x-auto no-scrollbar">

                <div class="relative w-full sm:w-48 flex-none">
                    <div class="absolute inset-y-0 left-0 pl-2.5 flex items-center pointer-events-none">
                        <i class="fas fa-search text-slate-400 text-xs"></i>
                    </div>
                    <input wire:model.live.debounce.300ms="search" type="text"
                        class="pl-8 w-full border-slate-200 rounded-lg text-xs font-bold text-slate-700 focus:ring-emerald-500 py-2 shadow-sm placeholder-slate-400"
                        placeholder="Cari Produk...">
                </div>

                <div class="relative w-36 flex-none" x-data="{ open: false }">
                    <button @click="open = !open" @click.outside="open = false"
                        class="w-full flex items-center justify-between bg-white border border-slate-200 text-slate-700 px-3 py-2 rounded-lg text-xs font-bold shadow-sm hover:border-emerald-300">
                        <span
                            class="truncate">{{ empty($filterCabang) ? 'Semua Cabang' : count($filterCabang).' Dipilih' }}</span>
                        <i class="fas fa-chevron-down text-[10px] text-slate-400"></i>
                    </button>
                    <div x-show="open"
                        class="absolute z-50 mt-1 w-full bg-white border border-slate-200 rounded-lg shadow-xl p-2 max-h-60 overflow-y-auto min-w-[150px]"
                        style="display: none;">
                ...
                <div class="relative w-36 flex-none" x-data="{ open: false }">
                    ...
                    <div x-show="open"
                        class="absolute z-50 mt-1 w-full bg-white border border-slate-200 rounded-lg shadow-xl p-2 max-h-60 overflow-y-auto min-w-[200px]"
                        style="display: none;">
                ...
                <button wire:click="openImportModal"
                    class="px-3 py-2 bg-emerald-600 text-white rounded-lg text-xs font-bold hover:bg-emerald-700 shadow-md shadow-emerald-500/20 transition-all flex items-center gap-2 flex-none">
    ...
    <div class="flex-1 bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col overflow-hidden min-h-0">
            ...
            <table class="text-[10px] text-left border-collapse whitespace-nowrap min-w-max w-full">
                <thead
                    ...
                    <tr>
                        ...
                        <th class="px-3 py-3 border-r border-slate-200 bg-slate-50 sticky left-[60px] z-30 shadow-sm">
                            C-Code</th>
                        <th class="px-3 py-3 border-r border-slate-200 bg-slate-50 sticky left-[120px] z-30 shadow-sm">
                            SKU</th>
                        ...
                        <th class="px-3 py-3 border-r border-slate-200 bg-slate-50">Expired</th>
                        <th
                        ...
                        <th class="px-3 py-3 border-r border-slate-200 bg-slate-50 text-right">Good Amt</th>
                        ...
                        <th class="px-3 py-3 border-r border-slate-200 bg-slate-50">Not Move</th>
                        ...
                        <th class="px-3 py-3 border-r border-slate-200 bg-slate-50 text-right">Bad Amt</th>
                        ...
                        <th class="px-3 py-3 border-r border-yellow-200 bg-yellow-50 text-right text-yellow-700">Margin
                        </th>
                        <th class="px-3 py-3 border-r border-yellow-200 bg-yellow-50 text-right text-yellow-700">%</th>
                        ...
                        <th class="px-3 py-3 border-r border-slate-200 bg-slate-50">Last Supp</th>
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    ...
                    <tr class="hover:bg-emerald-50/20 transition-colors group odd:bg-white even:bg-slate-50/30">
                    ...
                    <tr>
                        <td colspan="53" class="px-6 py-20 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <h3 class="text-slate-900 font-bold text-base">Data Produk Kosong</h3>
                                <p class="text-slate-400 text-xs mt-1 mb-4">Belum ada data produk yang diimport atau
                                    sesuai filter.</p>
                                <button wire:click="openImportModal"
        ...
        <div class="flex-none px-6 py-4 border-t border-slate-200 bg-slate-50/50">
            {{ $produks->links() }}
        </div>
    </div>

    @if($isImportOpen)
    @include('livewire.partials.import-modal', ['title' => 'Import Data Produk', 'color' => 'emerald'])
    @endif
