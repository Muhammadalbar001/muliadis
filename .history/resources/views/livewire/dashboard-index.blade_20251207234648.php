<div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row gap-4 items-end">
            <div class="w-full md:w-auto">
                ...
                <div class="flex items-center gap-2">
                    <input type="date" wire:model.live="startDate" class="text-sm border-gray-200 rounded-lg focus:ring-indigo-500">
                    <span class="text-gray-400">-</span>
                    <input type="date" wire:model.live="endDate" class="text-sm border-gray-200 rounded-lg focus:ring-indigo-500">
                </div>
            </div>

            <div class="w-full md:w-48">
                <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">Cabang</label>
                <select wire:model.live="filterCabang" class="w-full text-sm border-gray-200 rounded-lg focus:ring-indigo-500">
            ...
            <div class="w-full md:w-48">
                ...
                <select wire:model.live="filterSales" class="w-full text-sm border-gray-200 rounded-lg focus:ring-indigo-500">
            ...
            <div class="ml-auto" wire:loading>
                <span class="text-xs text-indigo-600 font-bold"><i class="fas fa-spinner fa-spin mr-1"></i> Memuat Data...</span>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-xl p-4 border border-red-100 shadow-sm">
            ...
            <span class="text-[10px] bg-red-50 text-red-600 px-2 py-0.5 rounded font-bold">Rasio: {{ number_format($persenRetur, 1) }}%</span>
        </div>
        <div class="bg-white rounded-xl p-4 border border-orange-100 shadow-sm">
            ...
            <h3 class="text-xl font-bold text-orange-600 mt-1">Rp {{ number_format($arSum / 1000000, 1, ',', '.') }} Jt</h3>
        ...
        <div class="bg-white rounded-xl p-4 border border-emerald-100 shadow-sm">
            ...
            <h3 class="text-xl font-bold text-emerald-600 mt-1">Rp {{ number_format($collSum / 1000000, 1, ',', '.') }} Jt</h3>
...
<script>
    document.addEventListener('livewire:init', () => {
        ...
        const optionsSales = {
            series: [{ name: 'Penjualan', data: @json($dataSales) }, { name: 'Retur', data: @json($dataRetur) }],
            chart: { type: 'area', height: 300, toolbar: { show: false }, fontFamily: 'Inter, sans-serif' },
            colors: ['#4f46e5', '#ef4444'],
            dataLabels: { enabled: false },
            stroke: { curve: 'smooth', width: 2 },
            xaxis: { categories: @json($chartDates), labels: { show: false } }, 
            yaxis: { labels: { formatter: (val) => { return (val/1000000).toFixed(0) + " Jt" } } },
        ...
        const optionsAR = {
            series: [{ name: 'Tagihan (AR)', data: @json($dataAR) }, { name: 'Bayar (Coll)', data: @json($dataColl) }],
        ...
        const optionsProd = {
            ...
            dataLabels: { enabled: true, textAnchor: 'start', style: { colors: ['#fff'] }, formatter: function (val, opt) { return val + " Unit" } },
            xaxis: { categories: topProdNames },
            colors: ['#3b82f6'],
            tooltip: { y: { formatter: (val) => { return new Intl.NumberFormat('id-ID').format(val) + " Unit" } } }
        };
        ...
        const optionsCust = {
            series: [{ name: 'Total Pembelian', data: topCustVal }],
            chart: { type: 'bar', height: 350, toolbar: { show: false }, fontFamily: 'Inter, sans-serif' },
            plotOptions: { bar: { borderRadius: 4, horizontal: true, barHeight: '70%' } },
            dataLabels: { enabled: true, textAnchor: 'start', formatter: function (val, opt) { return (val/1000000).toFixed(1) + " Jt" } },
            xaxis: { categories: topCustNames },
            colors: ['#8b5cf6'],
