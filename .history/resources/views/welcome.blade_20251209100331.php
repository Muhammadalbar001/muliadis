...
<body class="antialiased bg-white text-slate-600 selection:bg-indigo-500 selection:text-white"
    ...
    <nav class="fixed w-full z-50 transition-all duration-300" x-data="{ scrolled: false, mobileMenuOpen: false }"
        @scroll.window="scrolled = (window.pageYOffset > 20)"
        :class="scrolled ? 'bg-white/80 backdrop-blur-md border-b border-slate-100 shadow-sm py-2' : 'bg-transparent border-transparent py-4'">
            ...
            <div class="flex justify-between items-center h-16">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-[9px] font-bold text-slate-500 tracking-wider uppercase">Distribution System</p>
                    </div>
                </div>
                ...
                <div class="hidden md:flex items-center gap-3">
                    ...
                    <a href="{{ url('/dashboard') }}"
                        class="px-5 py-2 text-sm font-bold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 transition-all shadow-md shadow-indigo-200 hover:-translate-y-0.5">
                        Dashboard <i class="fas fa-arrow-right ml-2"></i>
                    </a>
                    @else
                    <a href="{{ route('login') }}" class="px-5 py-2 text-sm font-bold rounded-xl transition-colors"
                        :class="scrolled ? 'text-slate-600 hover:text-indigo-600' : 'text-slate-700 hover:text-indigo-600'">
                        Masuk
                    </a>
                    @if (Route::has('register'))
                    <a href="{{ route('register') }}"
                        class="px-5 py-2 text-sm font-bold text-white bg-slate-900 rounded-xl hover:bg-slate-800 transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5">
                        Daftar
                    </a>
                    @endif
                    @endauth
    ...
    <footer class="bg-slate-950 text-white py-6 border-t border-slate-900">
        <div class="max-w-7xl mx-auto px-6 lg:px-8">
            <div class="flex flex-col md:flex-row justify-between items-center gap-4">

                <div class="flex items-center gap-2">
                    <div
                        class="w-6 h-6 rounded bg-indigo-600 flex items-center justify-center text-white text-xs shadow-md shadow-indigo-500/20">
                        <i class="fas fa-truck-fast"></i>
                    </div>
                    <span class="font-bold text-sm tracking-wide text-slate-200">Muliadis App</span>
                </div>

                <div class="text-slate-500 text-[10px] sm:text-xs text-center md:text-right leading-tight">
                    <p>&copy; {{ date('Y') }} PT Mulia Anugerah Distribusindo.</p>
