    ...
    <div
        ...
        <div class="flex flex-col xl:flex-row gap-4 items-center justify-between">
            <div class="flex items-center gap-4 w-full xl:w-auto">
                <div class="p-2 bg-rose-100 rounded-lg text-rose-600 shadow-sm"><i class="fas fa-undo-alt text-xl"></i>
                </div>
                <div>
                    <h1 class="text-xl font-extrabold text-rose-900 tracking-tight">Rekap Retur</h1>
                    <p class="text-xs text-rose-600 font-medium mt-0.5">Lengkap Sesuai Excel ({{ $returs->total() }}
                        Baris)</p>
                </div>
            </div>
                    ...
                    <div x-show="open" x-transition
                        ...
                        <div @click="selected.includes('{{ $c }}') ? selected = selected.filter(i => i !== '{{ $c }}') : selected.push('{{ $c }}')"
                            ...
                            <div class="w-4 h-4 rounded border flex items-center justify-center mr-2"
                                :class="selected.includes('{{ $c }}') ? 'bg-rose-500 border-rose-500' : 'border-slate-300'">
                ...
                <div class="relative w-full sm:w-32" x-data="{ open: false, selected: @entangle('filterSales').live }">
                    ...
                    <div x-show="open" x-transition
                        ...
                        <div @click="selected.includes('{{ $s }}') ? selected = selected.filter(i => i !== '{{ $s }}') : selected.push('{{ $s }}')"
                            class="flex items-center px-2 py-1.5 hover:bg-rose-50 rounded cursor-pointer transition-colors group">
                            <div class="w-4 h-4 rounded border flex items-center justify-center mr-2"
                                :class="selected.includes('{{ $s }}') ? 'bg-rose-500 border-rose-500' : 'border-slate-300'">
            ...
            <table class="text-[10px] text-left border-collapse whitespace-nowrap min-w-max w-full">
                <thead
                    ...
                    <tr>
                        ...
                        <th class="px-3 py-3 border-r border-slate-200">Tgl Retur</th>
                        ...
                        <th class="px-3 py-3 border-r border-slate-200 text-right">Pembulatan</th>
                        ...
                        <th class="px-3 py-3 border-r border-slate-200 text-right">Disc 1</th>
                        <th class="px-3 py-3 border-r border-slate-200 text-right">Disc 2</th>
                        <th class="px-3 py-3 border-r border-slate-200 text-right">Disc Bawah</th>
                        <th class="px-3 py-3 border-r border-slate-200 text-right text-rose-600">Total Disc</th>
                        <th class="px-3 py-3 border-r border-slate-200 text-right">Nilai Net</th>
                        <th class="px-3 py-3 border-r border-slate-200 text-right">Total Harga</th>
                        <th class="px-3 py-3 border-r border-slate-200 text-right">PPN Head</th>
                        <th
                            class="px-3 py-3 border-r border-slate-200 text-right font-bold bg-yellow-50/50 text-yellow-700">
                            Total Grand</th>
                        <th class="px-3 py-3 border-r border-slate-200 text-right">PPN Value</th>
                        <th class="px-3 py-3 border-r border-slate-200 text-right">Min PPN</th>
                        ...
                        <th class="px-3 py-3 border-r border-slate-200">Last Supp</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 bg-white">
                    ...
                    <tr class="hover:bg-rose-50/20 transition-colors odd:bg-white even:bg-slate-50/30">
                        ...
                        <td class="px-3 py-2 border-r border-slate-100 text-right font-bold text-rose-700">
                    ...
                    @empty
                    <tr>
                        <td colspan="40" class="px-6 py-12 text-center text-slate-400">Data tidak ditemukan</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
