                ...
                <div class="flex flex-col gap-2">
                    <div class="flex flex-col md:flex-row gap-2 items-center">
                        <input type="file" wire:model="file" id="upload_{{ $iteration }}"
                            ...
                            class="block w-full text-xs text-slate-500 file:mr-2 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:bg-blue-600 file:text-white hover:file:bg-blue-700 cursor-pointer"
                            accept=".xlsx,.csv,.xls" />
                        ...
                        <button type="button" wire:click="import" wire:loading.attr="disabled"
                            ...
                            <span wire:loading.remove wire:target="import" x-show="!isUploading"><i
                                    class="fas fa-upload"></i> Import Retur</span>
                            <span x-show="isUploading" x-cloak><i class="fas fa-spinner fa-spin"></i> <span
            ...
            <table class="w-full text-sm text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-blue-50 transition duration-150">
                        ...
                        <td class="px-4 py-3 font-bold text-blue-700 whitespace-nowrap">{{ $item->no_retur }}<div
                                class="text-xs text-gray-400 font-normal">{{ $item->cabang }}</div>
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap text-gray-600 font-mono text-xs">{{ $item->no_inv }}</td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3 max-w-xs truncate" title="{{ $item->nama_item }}"><span
                                ...
                                class="text-xs text-gray-500 bg-gray-100 px-1 rounded">{{ $item->kode_item }}</span>
                        </td>
                        <td class="px-4 py-3 text-center"><span
                                class="bg-red-100 text-red-800 text-xs font-semibold px-2.5 py-0.5 rounded">{{ $item->qty }}
                                {{ $item->satuan_retur }}</span></td>
                    ...
                    @empty
                    <tr>
                        <td colspan="9" class="px-6 py-12 text-center text-gray-400">Tidak ada data retur.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        <div class="p-3 border-t bg-gray-50">{{ $retur->links() }}</div>
    </div>
</div>